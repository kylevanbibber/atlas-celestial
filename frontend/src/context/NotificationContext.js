...
import api from '../api';
import { playNotificationSound, playSaleSound } from '../utils/notificationSound';
...
const NotificationProvider = ({ children }) => {
            ...
            ws.onmessage = (event) => {
              try {
                ...
                if (data.type === 'auth_success') {
                  ...
                  // Update read status and decrement count together using fresh state
                  setNotifications(prevNotifications => {
                    const target = prevNotifications.find(n => n.id === data.notificationId);
                  ...
                  // Remove dismissed notification and decrement count together using fresh state
                  setNotifications(prevNotifications => {
                    const target = prevNotifications.find(n => n.id === data.notificationId);
                    if (target && !target.is_read) {
                      setUnreadCount(prevCount => Math.max(0, prevCount - 1));
                    }
                    return prevNotifications.filter(n => n.id !== data.notificationId);
                  });
                  
                } else if (data.type === 'new_close') {
                  // New sale broadcast — show toast, play sound, dispatch event for ActivityFeed
                  if (data.event) {
                    // Format actor name for display
                    const name = data.event.actor?.name || 'Someone';
                    ...
                    const displayName = parts.length >= 2
                      ...
                      : name;
                    const alp = data.event.data?.alp ? `$${Number(data.event.data.alp).toLocaleString()}` : '';
                    ...
                    if (document.visibilityState === 'visible' && !document.hidden) {
                      if (window.showInPageNotification) {
                        window.showInPageNotification({
                          id: 'sale-' + Date.now(),
                          title: 'New Sale!',
                          message: `${displayName} closed ${alp} ALP`,
                          link_url: '/activity'
                        });
                      }
                      playSaleSound();
                    ...
                    window.dispatchEvent(new CustomEvent('new_close', { detail: data.event }));
                  }

                } else if (data.type === 'text_campaign_message') {
                  window.dispatchEvent(new CustomEvent('text_campaign_message', { detail: data }));

                } else if (data.type === 'text_campaign_status') {
                  window.dispatchEvent(new CustomEvent('text_campaign_status', { detail: data }));
    ...
    const checkSubscriptionSync = async () => {
      ...
      try {
        ...
        if (existingSubscription) {
          // Browser has a subscription - make sure it's synced to the database
          setPushSubscription(existingSubscription);
          
          // Re-register with backend to ensure this device's subscription is current
          try {
            ...
            // Silent sync failure is OK - subscription may already be saved
          }
        } else if (permissionGranted) {
          // Permission is granted but no browser subscription exists
          // This happens when service worker updates, browser clears data, or
          // switching devices where permission was previously granted
          // Re-create the subscription
          subscribeToPushNotifications();
        }
        // If permission is 'default' or 'denied', don't auto-subscribe
        // The PushNotificationPrompt will handle prompting the user
      } catch (err) {
  ...
  }, [vapidPublicKey, isAuthenticated, subscribeToPushNotifications, user?.userId]);
