...
const hasDashboardAccess = (user) => {
  ...
  // App admin users (teamRole='app') access dashboard via AdminHome
  if (user.teamRole === 'app') return true;
...
export const AuthProvider = ({ children }) => {
  ...
  /**
   * Silently refresh the auth token by calling POST /auth/refresh.
  ...
  const refreshToken = useCallback(async () => {
    ...
    try {
      ...
      console.warn('[AuthContext] ⚠️ Token refresh failed:', err?.response?.status || err.message);
      // If we get a 401 the token is truly dead — don't force-logout here;
      // the periodic check below will handle that.
    }
    return null;
  }, [token]);

  // Periodic token validation check + silent refresh
  useEffect(() => {
    let tokenCheckInterval;
    // 7 days in seconds — threshold for proactive refresh
    ...
    const checkAndRefresh = async () => {
      ...
      // If token will expire within 7 days, silently refresh
      if (timeUntilExpiry > 0 && timeUntilExpiry <= REFRESH_THRESHOLD) {
        console.log(`[AuthContext] 🔄 Token expires in ${Math.round(timeUntilExpiry / 86400)} day(s), refreshing...`);
        await refreshToken();
      }
    };
    ...
    if (token && user) {
      ...
      const initialTimer = setTimeout(checkAndRefresh, 5000);

      // Then check every 6 hours
      tokenCheckInterval = setInterval(checkAndRefresh, 6 * 60 * 60 * 1000);
  ...
  }, [token, user, logout, refreshToken]);

  // PWA / mobile: refresh token when the app comes back to the foreground
  useEffect(() => {
    if (!token || !user) return;

    const handleVisibilityChange = async () => {
      if (document.visibilityState === 'visible') {
        console.log('[AuthContext] 👁️ App became visible, checking token...');
        const decoded = decodeToken(token);

        if (!decoded) {
          // Token completely expired while app was in the background
          console.log('[AuthContext] 🚪 Token expired while app was backgrounded');
          const currentPath = window.location.pathname + window.location.search + window.location.hash;
          const isAuth = ['/login', '/register', '/adminlogin', '/onboarding/login'].includes(window.location.pathname);
        ...
        if (timeUntilExpiry > 0 && timeUntilExpiry <= REFRESH_THRESHOLD) {
          console.log(`[AuthContext] 🔄 Token close to expiry (${Math.round(timeUntilExpiry / 86400)}d left), refreshing on resume...`);
          await refreshToken();
        }
      }
    };
  ...
  const hasPermission = useCallback((permission) => {
    ...
    // App admin users (teamRole='app') get dashboard access
    if (user.teamRole === 'app' && permission === 'view_dashboard') {
      return true;
    }
