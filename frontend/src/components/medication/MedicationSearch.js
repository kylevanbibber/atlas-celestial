...
const MedicationSearch = () => {
  ...
  const conditionQuestions = {
    ...
    'Weight': [
      ...
      { id: 'coverageAmount', question: 'Coverage amount requested', type: 'select', options: ['Under 120k', 'Over 120k', 'Senior Graded'] }
    ],
    'Recent Surgery': [
  ...
  const calculateWeightTable = (heightFeet, heightInches, weight, gender) => {
    ...
    // Weight table data based on the image
    // Format: height in inches -> [T2, T3, T4, T5, T6, T8, T10, T12]
    const weightTables = {
      male: {
        48: [80, 118, 170, 174, 178, 186, 191, 195],
        49: [82, 121, 174, 178, 182, 190, 195, 199],
        50: [83, 123, 177, 182, 186, 194, 199, 203],
        51: [85, 126, 181, 185, 190, 198, 203, 207],
        52: [87, 128, 185, 189, 193, 202, 207, 212],
        53: [88, 131, 188, 192, 197, 206, 211, 216],
        54: [90, 133, 192, 196, 201, 210, 215, 220],
        55: [92, 136, 195, 200, 204, 214, 219, 224],
    ...
    // Use male table for both genders (female-specific tables can be added later)
    const table = weightTables.male[totalInches];
    if (table) {
      if (weightNum < table[0]) return 1;
      if (weightNum <= table[1]) return 2;
      if (weightNum <= table[2]) return 3;
      if (weightNum <= table[3]) return 4;
      if (weightNum <= table[4]) return 5;
      if (weightNum <= table[5]) return 6;
      if (weightNum <= table[6]) return 8;
      if (weightNum <= table[7]) return 10;
      return 12;
    }
  ...
  const calculateConditionTrial = (condition) => {
    ...
    switch (condition.condition) {
      ...
      case 'Weight':
        ...
        if (coverage === 'Over 120k' && table >= 3) {
          return { isTrial: true, reason: 'Table 3+ for coverage over 120k' };
        }
        if (table >= 4) {
