...
const formatRelativeTime = (timestamp) => {
  ...
  // Parse the timestamp - MySQL stores in UTC, convert to Eastern for display
  let dateTime;
  if (timestamp && typeof timestamp === 'string' && timestamp.includes('T')) {
    // Already in ISO format (e.g. from JS Date.toISOString()) - treat as UTC
    dateTime = DateTime.fromISO(timestamp, { zone: 'utc' }).setZone('America/New_York');
  } else {
    // MySQL format (YYYY-MM-DD HH:MM:SS) - server stores UTC
    const safeStr = String(timestamp || '').trim();
    dateTime = safeStr
      ? DateTime.fromSQL(safeStr, { zone: 'utc' }).setZone('America/New_York')
      : DateTime.invalid('empty');
  }
