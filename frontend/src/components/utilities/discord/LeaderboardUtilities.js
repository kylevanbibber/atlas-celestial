...
const LeaderboardUtilities = ({ leaderboards, setLeaderboards, configuredGuilds, availableManagers, userRole }) => {
  const [showLeaderboardModal, setShowLeaderboardModal] = useState(false);

  const handleCreateLeaderboard = async (formData) => {
    console.log('🚀 handleCreateLeaderboard called with data:', formData);
    try {
      console.log('📤 Sending POST request to /discord/leaderboards...');
      ...
      alert(`Failed to create leaderboard: ${error.response?.data?.error || error.message}`);
    }
  };
  ...
  const getManagerLabel = (scope) => {
    switch (scope) {
      case 'mga_team': return 'MGA Leader';
      case 'rga_team': return 'RGA Leader';
      case 'family_tree': return 'Family Tree Leader';
      case 'agency_tree': return 'Agency Leader';
      case 'full_agency': return 'Full Agency Leader';
      default: return 'Leader (Team Owner)';
    }
  };
...
const LeaderboardItem = ({ leaderboard, onUpdate, configuredGuilds, availableManagers, userRole }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    week_start_day: leaderboard.week_start_day || 'monday',
          ...
          <div className="form-row">
            <div className="form-group">
              ...
              <select
                ...
                onChange={(e) => setFormData({...formData, week_start_day: e.target.value})}
                className="settings-row select"
              >
              ...
              <div style={{ fontSize: '0.8em', color: 'var(--text-secondary)', marginTop: '4px' }}>
                Determines the week start day for weekly data periods
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', flex: 1 }}>
            ...
            {userRole === 'Admin' && (
              <div style={{ 
                ...
                <span>
                  <strong>Leader:</strong> {leaderboard.manager_lagnname || 'Unknown'} 
...
const InlineLeaderboardForm = ({ configuredGuilds, onSubmit, onCancel, availableManagers, userRole }) => {
  const [formData, setFormData] = useState({
    ...
    week_start_day: 'monday', // monday or wednesday
    top_count: 10
  });
  const [availableChannels, setAvailableChannels] = useState([]);
  const [loadingChannels, setLoadingChannels] = useState(false);
  ...
  const handleSubmit = (e) => {
    ...
    const leaderboardData = {
      guild_id: formData.guild_id,
      guild_name: selectedGuild?.guild_name || 'Unknown Server',
      channel_id: formData.channel_id,
      channel_name: selectedChannel?.name || 'Unknown Channel',
      cron_expr: generateCronExpression(formData.frequency),
      metric_type: formData.leaderboard_type === 'production' ? 'production_leaderboard' : 'activity_leaderboard',
      leaderboard_type: formData.leaderboard_type,
      metrics: formData.metrics,
    ...
    console.log('Calling onSubmit with:', leaderboardData);
  ...
  return (
    <div>
      ...
      <div className="form-actions">
        ...
        <button type="button" onClick={handleSubmit} className="settings-button">
          Create Leaderboard
        </button>
      </div>
    </div>
