...
const AllotmentTab = () => {
  ...
  const [issuedData, setIssuedData] = useState({}); // Map of "agentId-dropDateId-leadType" to quantity
  ...
  // Check if user can edit drop issuance data (Admin or app only)
  const canEditIssuedData = useMemo(() => {
    if (!user) return false;
    return user.Role === 'Admin' || user.teamRole === 'app';
  }, [user]);
  ...
  const fetchAllotmentData = useCallback(async () => {
    ...
    try {
      ...
      // Handle issued data - build lookup map
      const issuedRaw = response.data?.issuedData || [];
      const issuedMap = {};
      issuedRaw.forEach(record => {
        const key = `${record.agent_id}-${record.drop_date_id}-${record.lead_type}`;
        issuedMap[key] = record.quantity;
      });
  ...
  const { highAlpData, lowAlpData, effectiveF90Ids } = useMemo(() => {
    ...
    // STEP 1: Sort ALL high ALP agents by ALP descending (including F90-eligible)
    const allHighAlpSorted = [...highAlpAgents].sort((a, b) => (b.alp || 0) - (a.alp || 0));

    // STEP 2: Tentatively assign groups to determine where F90 agents would land
    const tentativeTotal = allHighAlpSorted.length;
    const tentativeBase = Math.floor(tentativeTotal / 5);
    const tentativeRemainder = tentativeTotal % 5;

    const tentativeGroups = allHighAlpSorted.map((agent, index) => {
      let group = 1;
      let currentIndex = 0;
      for (let g = 1; g <= 5; g++) {
        const groupSize = tentativeBase + (g <= tentativeRemainder ? 1 : 0);
      ...
      return { ...agent, tentativeGroup: group };
    });

    // STEP 3: F90-eligible agents in Groups 1-3 stay in regular allotment;
    //         F90-eligible agents in Groups 4-5 go to F90
    const effectiveF90Set = new Set();
    if (f90AgentIds && f90AgentIds.size > 0) {
      tentativeGroups.forEach(agent => {
        if (f90AgentIds.has(agent.agentId) && agent.tentativeGroup >= 4) {
          effectiveF90Set.add(agent.agentId);
        }
      });

      const keptInGroups = Array.from(f90AgentIds).filter(id =>
        highAlpAgents.some(a => a.agentId === id) && !effectiveF90Set.has(id)
      );
      console.log(`🔄 [F90] ${effectiveF90Set.size} F90 agents → F90 tab (would be Groups 4-5)`);
      console.log(`🔄 [F90] ${keptInGroups.length} F90 agents → kept in regular groups (Groups 1-3)`);
    }

    // STEP 4: Remove effective F90 agents and redistribute remaining into 5 groups
    const nonF90HighAlpAgents = allHighAlpSorted.filter(agent => !effectiveF90Set.has(agent.agentId));
    const totalAgents = nonF90HighAlpAgents.length;
    const baseGroupSize = Math.floor(totalAgents / 5);
    const remainder = totalAgents % 5;

    console.log(`🔢 [Frontend] Distributing ${totalAgents} agents into 5 groups: ${baseGroupSize} base + ${remainder} extra`);
    ...
    // STEP 5: Apply manual group overrides
    ...
    return {
      ...
      effectiveF90Ids: effectiveF90Set
    };
  ...
  const filteredHighAlpData = useMemo(() => {
    ...
    // Check if filtering by F90 (agents from leads_released with 1st Pack 31-90 days before drop date)
      ...
      console.log('🔍 [F90] Starting F90 filter...', {
        ...
        f90EligibleFromBackend: f90AgentIds ? f90AgentIds.size : 0,
        effectiveF90: effectiveF90Ids ? effectiveF90Ids.size : 0,
        dropDatesCount: dropDates.length
      });

      // Filter to effective F90 agents (would-be Group 4-5, >= 3000 ALP)
      filtered = allotmentData.filter(item => {
        if (item.isExcluded) return false;
        return effectiveF90Ids && effectiveF90Ids.has(item.agentId);
      });
    ...
    return filtered;
  }, [highAlpData, lowAlpData, allotmentData, searchQuery, activeGroupFilter, customGroups, getGroupDetails, effectiveF90Ids, f90AgentData, dropDates]);
  ...
  // Save a single lead issuance value
  const handleIssuedSave = useCallback(async (agentId, dropDateId, leadType, quantity) => {
    try {
      await api.put('/pnp/allotment-issued', { agentId, dropDateId, leadType, quantity });
      // Update local state optimistically
      setIssuedData(prev => ({
        ...prev,
        [`${agentId}-${dropDateId}-${leadType}`]: quantity
      }));
    } catch (error) {
      console.error('Error saving issuance:', error);
      toast.error('Failed to save lead count');
    }
  }, []);
  ...
  const highAlpColumns = useMemo(() => {
    ...
    },
    {
      ...
      Cell: ({ row }) => {
        ...
        if (vipCount === 0) {
          return <span style={{ color: 'var(--text-secondary, #999)' }}>-</span>;
        }
          ...
          return (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', fontSize: '11px' }}>
              ...
              {row.original.groupOverridden && (
                <span style={{ fontSize: '9px', color: 'var(--text-secondary, #666)', fontStyle: 'italic' }}>
    ...
    // Add lead issuance columns for each drop date (flat — no grouped headers)
    const leadTypes = ['POS', 'HC', 'VN', 'Dcard'];
    dropDates.forEach((drop, idx) => {
      const dropLabel = `Drop ${idx + 1}`;

      // First column: drop name header, shows total
      columns.push({
        Header: dropLabel,
        id: `issued-${drop.id}-total`,
        accessor: () => null,
        Cell: ({ row }) => {
          const agentId = row.original.agentId;
          const total = leadTypes.reduce((sum, lt) => {
            return sum + (issuedData[`${agentId}-${drop.id}-${lt}`] || 0);
          }, 0);
          ...
          return (
            <span style={{
              fontWeight: '600',
              fontSize: '12px',
              color: total > 0 ? '#047857' : 'var(--text-secondary, #999)'
            }}>
              {total}
            </span>
          );
      ...
      // Individual lead type columns
      leadTypes.forEach(lt => {
        columns.push({
          Header: lt,
          id: `issued-${drop.id}-${lt}`,
          accessor: () => null,
          Cell: ({ row }) => {
            const agentId = row.original.agentId;
            const key = `${agentId}-${drop.id}-${lt}`;
            const val = issuedData[key] ?? '';

            if (!canEditIssuedData) {
            ...
            return (
              <input
                ...
                onBlur={(e) => {
                  ...
                  if (qty !== (issuedData[key] || 0)) {
                    handleIssuedSave(agentId, drop.id, lt, qty);
                  }
                }}
                style={{
                  width: '50px',
    ...
    return columns;
  }, [activeGroupFilter, customGroups, hasElevatedPermissions, canEditIssuedData, handleRemoveOverride, handleAddOverride, f90AgentData, dropDates, issuedData, handleIssuedSave]);
  ...
  const lowAlpColumns = useMemo(() => [
    ...
    },
    {
      ...
      Cell: ({ value }) => {
        ...
        const bgColor = num > 0 ? 'rgba(0, 85, 140, 0.08)' : 'transparent';
    ...
    <div>
      ...
      <div className="search-bar mb-4" style={{ marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'space-between', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <label className="allotment-toolbar-label">
            Allotment Month:
          </label>
          <select
            value={selectedMonth}
            onChange={(e) => setSelectedMonth(e.target.value)}
            className="allotment-search-select"
          ...
          {apiResponseData && (
            <div className="allotment-toolbar-divider" style={{
              fontSize: '11px',
              paddingLeft: '12px',
              display: 'flex',
      ...
      {showBulkActionsModal && (
        <div className="allotment-bulk-modal-overlay">
          <div className="allotment-bulk-modal">
      ...
      <div style={{ marginBottom: '40px', position: 'relative' }}>
        {/* Loading Overlay */}
        {loading && (
          <div className="allotment-loading-overlay" style={{ borderRadius: '8px' }}>
            <div style={{
              width: '50px',
              height: '50px',
        ...
        <div className="allotment-tab-bar" style={{
          marginBottom: '15px',
          paddingBottom: '0px'
        }}>
          <div style={{ 
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: isActive ? 'var(--bg-secondary, #f8f9fa)' : 'transparent',
                    color: isActive ? customGroup.color : 'var(--text-secondary, #6c757d)',
                  ...
                  onMouseEnter={(e) => {
                    if (!isActive) {
                      e.currentTarget.style.backgroundColor = 'var(--bg-tertiary, #f1f3f5)';
                      e.currentTarget.style.color = 'var(--text-primary, #495057)';
                    }
                  }}
                  onMouseLeave={(e) => {
                    if (!isActive) {
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: isActive ? 'var(--bg-secondary, #f8f9fa)' : 'transparent',
                    color: isActive ? '#2c5aa0' : 'var(--text-secondary, #6c757d)',
                    fontWeight: isActive ? 'bold' : 'normal',
                    fontSize: '13px',
                    cursor: 'pointer',
                    transition: 'all 0.2s ease',
            ...
            {(() => {
              ...
              // Count effective F90 agents (would-be Group 4-5, >= 3000 ALP)
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: isActive ? 'var(--bg-secondary, #f8f9fa)' : 'transparent',
                    color: isActive ? '#047857' : 'var(--text-secondary, #6c757d)',
                    fontWeight: isActive ? 'bold' : 'normal',
                    fontSize: '13px',
                    cursor: 'pointer',
                    transition: 'all 0.2s ease',
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: isActive ? 'var(--bg-secondary, #f8f9fa)' : 'transparent',
                    color: isActive ? '#856404' : 'var(--text-secondary, #6c757d)',
                    fontWeight: isActive ? 'bold' : 'normal',
                    fontSize: '13px',
                    cursor: 'pointer',
                    transition: 'all 0.2s ease',
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: isActive ? 'var(--bg-secondary, #f8f9fa)' : 'transparent',
                    color: isActive ? '#f59e0b' : 'var(--text-secondary, #6c757d)',
                    fontWeight: isActive ? 'bold' : 'normal',
                    fontSize: '13px',
                    cursor: 'pointer',
                    transition: 'all 0.2s ease',
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: isActive ? 'var(--bg-secondary, #f8f9fa)' : 'transparent',
                    color: isActive ? '#dc2626' : 'var(--text-secondary, #6c757d)',
                    fontWeight: isActive ? 'bold' : 'normal',
                    fontSize: '13px',
                    cursor: 'pointer',
                    transition: 'all 0.2s ease',
