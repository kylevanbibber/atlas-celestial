...
const NotificationUtilities = () => {
  ...
  const [emailPrefs, setEmailPrefs] = useState({ daily_reminder: true, weekly_report: true });
  const [emailPrefsLoading, setEmailPrefsLoading] = useState(false);
  const [emailPrefsSaving, setEmailPrefsSaving] = useState(null); // which pref is saving
  ...
  useEffect(() => {
    const fetchEmailPrefs = async () => {
      try {
        ...
        const response = await api.get('/notifications/email-preferences');
  ...
  // Toggle an email preference
  const handleToggleEmailPref = async (prefType) => {
    try {
      setEmailPrefsSaving(prefType);
  ...
  return (
    <div className="settings-section">
      ...
      <h2>Notification Settings</h2>
        ...
        {/* Email Notifications */}
        {(isAdmin || (user?.clname && ['AGT', 'SA', 'GA', 'MGA', 'RGA'].includes(user.clname))) && (
          <>
            <h2 style={{ marginTop: '30px' }}>Email Notifications</h2>
            <p style={{ color: 'var(--text-secondary, #7f8c8d)', fontSize: '14px', marginBottom: '15px' }}>
              Control which automated emails you receive.
            </p>

            {/* Daily Activity Reminder - AGT, SA, GA (and admins) */}
            {(isAdmin || ['AGT', 'SA', 'GA'].includes(user?.clname)) && (
              <div className="settings-row theme-settings-row" style={{ padding: '12px 0', borderBottom: '1px solid var(--border-color, #e9ecef)' }}>
                <div style={{ flex: 1 }}>
                  <label style={{ fontWeight: 500, fontSize: '15px' }}>Daily Activity Reminders</label>
                  <div style={{ fontSize: '13px', color: 'var(--text-secondary, #7f8c8d)', marginTop: '4px' }}>
                    Receive a reminder email at 9 PM ET if you haven't submitted daily activity
                  </div>
                </div>
                <div className="switch-container">
                  <label className="switch">
                    <input
                      ...
                      onChange={() => handleToggleEmailPref('daily_reminder')}
                      disabled={emailPrefsLoading || emailPrefsSaving === 'daily_reminder'}
                    />
                    <span className="slider"></span>
                  </label>
                  <span className="switch-label">
                    {emailPrefsSaving === 'daily_reminder' ? 'Saving...' : (emailPrefs.daily_reminder ? 'On' : 'Off')}
                  </span>
                </div>
              </div>
            )}

            {/* Weekly Production Reports - MGA, RGA, SA, GA (and admins) */}
            {(isAdmin || ['MGA', 'RGA', 'SA', 'GA'].includes(user?.clname)) && (
              <div className="settings-row theme-settings-row" style={{ padding: '12px 0', borderBottom: '1px solid var(--border-color, #e9ecef)' }}>
                <div style={{ flex: 1 }}>
                  <label style={{ fontWeight: 500, fontSize: '15px' }}>Weekly Production Reports</label>
                  <div style={{ fontSize: '13px', color: 'var(--text-secondary, #7f8c8d)', marginTop: '4px' }}>
                    Receive a weekly production report email every Monday
                  </div>
                </div>
                <div className="switch-container">
                  <label className="switch">
                    <input
                      ...
                      onChange={() => handleToggleEmailPref('weekly_report')}
                      disabled={emailPrefsLoading || emailPrefsSaving === 'weekly_report'}
                    />
                    <span className="slider"></span>
                  </label>
                  <span className="switch-label">
                    {emailPrefsSaving === 'weekly_report' ? 'Saving...' : (emailPrefs.weekly_report ? 'On' : 'Off')}
                  </span>
                </div>
              </div>
            )}
      ...
      {/* Diagnostic component for troubleshooting - only show for user 92 */}
      {(user && (user.id === 92 || user.userId === 92 || user.user_id === 92)) && (
