...
import { FiX, FiPlus, FiUpload, FiDownload, FiTrash2, FiArchive, FiMail, FiRefreshCw, FiUsers, FiEye, FiEyeOff, FiFilter, FiSave, FiBookmark, FiInbox } from "react-icons/fi";
...
const DataTable = ({
  ...
  const handleHeaderMassUpdate = (columnAccessor, newValue) => {
    setLocalData((prev) =>
      prev.map((row) =>
        selectedRows[row.id] ? { ...row, [columnAccessor]: newValue } : row
      )
    );
    Object.keys(selectedRows).forEach((rowId) => {
      onCellUpdate(rowId, columnAccessor, newValue);
    });
  };
  ...
  if (enableColumnFilters && Object.keys(columnFilters).length > 0) {
    filteredData = filteredData.filter(row => {
      return Object.entries(columnFilters).every(([columnId, filterValue]) => {
        ...
        // Find the column config to check for filterKey/filterSplitBy
        const column = columns.find(col => col.accessor === columnId || col.id === columnId);
        const filterKey = column?.filterKey || columnId;
  ...
  const toggleSelectAll = () => {
    // Use filteredData so mass select only applies to visible (filtered) rows
    if (filteredData.length && Object.keys(selectedRows).length === filteredData.length) {
      setSelectedRows({});
                    ...
                    {headerGroup.headers.map((column) => (
                      <th
                        ...
                        {...(
                          column.massSelection
                            ? {}
                            : (column.id === "status" || column.chipDropdown) &&
                      ...
                      >
                        {column.massSelection ? (
                          ...
                          Object.keys(selectedRows).length > 1 &&
                          column.chipDropdownOptions?.length > 0 ? (
                          /* mass chipDropdown update in header */
                          <select
                            style={{
                              width: "100%",
                              boxSizing: "border-box",
                            ...
                            onChange={(e) => {
                              if (e.target.value) {
                                handleHeaderMassUpdate(column.id, e.target.value);
                                e.target.value = "";
                              }
                            }}
                            onClick={(e) => e.stopPropagation()}
                          >
                            <option value="">{column.render("Header")}</option>
                            {column.chipDropdownOptions.map((opt) => {
                              const val = opt[column.chipDropdownValueField] ?? opt;
                              const display = column.chipDropdownDisplay ? column.chipDropdownDisplay(opt) : String(val);
                              return (
                                ...
                                <button
                                  ...
                                  onClick={(e) => {
                                    ...
                                    if (showColumnFilter === column.id) {
                                      ...
                                      // Calculate position based on button location, keeping within viewport
                                      const rect = e.currentTarget.getBoundingClientRect();
                                      const dropdownWidth = 240;
                                      let left = rect.right - dropdownWidth; // align right edge to button
                                      // If it goes off the left edge, align left edge to button instead
                                      if (left < 8) {
                                        left = rect.left;
                                      }
                                      // If it goes off the right edge, clamp to viewport
                                ...
                                {showColumnFilter === column.id && (
                                  ...
                                  >
                                    {(() => {
                                      ...
                                      const filterKey = column.filterKey || column.id;
                                      const filterDisplayFn = column.filterDisplayFn || null;
                                      ...
                                      if (filterSplitBy) {
                                        ...
                                        uniqueValues = [...new Set(splitValues)].sort((a, b) => {
                                          const da = filterDisplayFn ? String(filterDisplayFn(a)) : String(a);
                                          const db = filterDisplayFn ? String(filterDisplayFn(b)) : String(b);
                                      ...
                                      const filteredValues = uniqueValues.filter(val => {
                                        const displayLabel = filterDisplayFn ? filterDisplayFn(val) : val;
                                        return String(displayLabel).toLowerCase().includes(filterSearchTerm.toLowerCase());
                                      });
                                              ...
                                              <div key={val} style={{ 
                                                ...
                                                <label style={{
                                                  ...
                                                  <span style={{
                                                    ...
                                                    whiteSpace: 'nowrap',
                                                    flex: 1
                                                  }}>{filterDisplayFn ? filterDisplayFn(val) : String(val)}</span>
                                                </label>
                                              </div>
                                            ))}
                                          </div>
                    ...
                    <td colSpan={flattenColumns(columns).length} className="no-data-cell">
                      <div className="no-data-container">
                        ...
                        <div className="no-data-message">
                          <span>No {entityName}s to display</span>
                        </div>
                      </div>
                    </td>
                  </tr>
