...
const DailyActivityReminder = ({ user }) => {
  ...
  const handleSubmit = async () => {
    ...
    const submitData = {
      // Match DailyActivityForm pattern - use userId with fallback to id
    ...
    console.log('[DailyActivityReminder] Submitting data:', JSON.stringify(submitData, null, 2));
    console.log('[DailyActivityReminder] User ID:', submitData.userId);
    console.log('[DailyActivityReminder] Update date:', yesterdayDate);
    console.log('[DailyActivityReminder] Form fields:', Object.keys(formData).filter(key => formData[key] !== ''));

    try {
      const response = await api.post('/dailyActivity/update', submitData);

      const result = response.data;
      
      console.log('[DailyActivityReminder] Response received:', result);

      if (result.success) {
        console.log('[DailyActivityReminder] ✓ Save successful');
        setShowSuccess(true);
        ...
        console.error('[DailyActivityReminder] ❌ Save failed:', result.message);
        alert('Failed to save activity. Please try again.');
      }
    } catch (error) {
      console.error('[DailyActivityReminder] ❌ Error saving activity:', error);
      console.error('[DailyActivityReminder] Error details:', error.response?.data);
