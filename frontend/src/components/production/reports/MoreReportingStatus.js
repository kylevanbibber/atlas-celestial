...
const MoreReportingStatus = ({ amoreData, filters, dateRange, allowedMGAs }) => {
  ...
  useEffect(() => {
    if (mgaData.length) {
      ...
      const filtered = mgaData.filter((mga) => {
        // If allowedMGAs list is provided, use it as the primary filter
        // (includes all MGAs/RGAs in the user's RGA hierarchy + their own)
        if (allowedMGAs && allowedMGAs.length > 0) {
          const allowed = allowedMGAs.map(n => n.toUpperCase());
          if (!allowed.includes((mga.lagnname || '').toUpperCase())) return false;
        } else {
  ...
  }, [filters, mgaData, amoreData, showOnlyNotReported, allowedMGAs]);
