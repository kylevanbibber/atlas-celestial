...
import { FiSend, FiMousePointer, FiCheckCircle, FiXCircle, FiClock, FiPlus, FiSearch, FiRefreshCw, FiCalendar, FiUser, FiMessageSquare, FiList } from 'react-icons/fi';
import '../ProductionReports.css';
import { debounce } from 'lodash';
import { toast } from 'react-hot-toast';
import VerifyConversations from './VerifyConversations';
...
const VerifyTableComponent = React.forwardRef(({ onOpenRightPanel }, ref) => {
    ...
    const [viewMode, setViewMode] = useState('table'); // 'table' or 'conversations'
    const [isArchiveView, setIsArchiveView] = useState(false);
    const [archiveMonthFrom, setArchiveMonthFrom] = useState(() => {
        const now = new Date();
        return `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
    });
    const [archiveMonthTo, setArchiveMonthTo] = useState(() => {
        const now = new Date();
        return `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
    });
    ...
    // Fetch agent list for archive dropdown
    useEffect(() => {
        const fetchAgents = async () => {
            try {
                const response = await api.get('/verify/agents');
    ...
    const fetchVerifyData = async (archiveView = isArchiveView) => {
        try {
            ...
            let queryString = `archive=${archiveParam}`;

            // Add month range and agent filters for archive view
            if (archiveView && archiveMonthFrom) {
                queryString += `&monthFrom=${archiveMonthFrom}`;
            }
            if (archiveView && archiveMonthTo) {
                queryString += `&monthTo=${archiveMonthTo}`;
            }
            if (archiveView && selectedAgent) {
                queryString += `&agent=${encodeURIComponent(selectedAgent)}`;
            }

            const [verifyResponse, verifyClientResponse] = await Promise.all([
                api.get(`/verify/all?${queryString}`),
                api.get(`/verify/verifyclient/all?${queryString}`)
            ]);
    ...
    }, [userIds, isAppAdmin, isArchiveView, archiveMonthFrom, archiveMonthTo, selectedAgent]);
    ...
    const getQueuedData = useCallback(() => {
        return verifyData.filter(
            (verifyRow) => {
                if (selectedAgent && verifyRow.agent_name !== selectedAgent) return false;
    ...
    }, [verifyData, verifyClientData, isArchiveView, isAdmin, selectedAgent]);
    ...
    }, [verifyData, verifyClientData, isArchiveView, isAdmin, computeDiscrepancyReasons, selectedAgent]);
    ...
    const getDiscrepancyData = useCallback(() => {
        return verifyData.map(verifyRow => {
            if (selectedAgent && verifyRow.agent_name !== selectedAgent) return null;
    ...
    const handleResend = useCallback(async (applicationId) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                toast.error(`Failed to resend: ${response.data.message || 'Unknown error'}`);
            }
        } catch (error) {
            console.error('Error resending:', error);
            toast.error(`Error resending verification: ${error.message}`);
        }
    }, [verifyData, hasAppManagementPermissions, refreshTabData]);
    ...
    const getRowContextMenuOptions = useCallback((row) => {
        ...
        // View Agent option - available in both archive and non-archive views
        if (row.agent_name) {
            options.push({
                label: 'View Agent',
                icon: '👤',
                onClick: () => setSelectedAgent(row.agent_name),
                className: 'menu-item-view-agent'
            });
        }
    ...
    // If in conversations view, render VerifyConversations
    if (viewMode === 'conversations') {
        return (
            <div style={{ position: 'relative' }}>
                {/* View mode toggle */}
                {isAppAdmin && (
                    <div className="verify-view-toggle">
                        <button
                            className={`verify-view-btn ${viewMode === 'table' ? 'active' : ''}`}
                        ...
                        <button
                            className={`verify-view-btn ${viewMode === 'conversations' ? 'active' : ''}`}
    ...
    return (
        <div style={{ position: 'relative' }}>
            ...
            <div className={`verify-layout ${selectedAgent ? 'with-stats-panel' : ''}`}>
                {selectedAgent && (
                    <AgentStatsPanel
                        selectedAgent={selectedAgent}
                        periodTabCounts={tabCounts}
                        periodVerifyData={verifyData}
                        computeDiscrepancyReasons={computeDiscrepancyReasons}
                        onClose={() => setSelectedAgent('')}
                    />
                )}
                <div className="verify-table-area">
                {/* Search Bar + Archive Filters in same row */}
                <div className="verify-search-row">
                    ...
                    {isArchiveView && (
                        <>
                            <div className="verify-archive-filter-group">
                                <FiCalendar size={14} />
                                <input
                                    type="month"
                                    value={archiveMonthFrom}
                                    onChange={(e) => {
                                        setArchiveMonthFrom(e.target.value);
                                        if (e.target.value > archiveMonthTo) setArchiveMonthTo(e.target.value);
                                    }}
                                    className="verify-archive-month-input"
                                    title="From month"
                                />
                                <span className="verify-archive-month-sep">to</span>
                                <input
                                    type="month"
                                    value={archiveMonthTo}
                                    onChange={(e) => {
                                        setArchiveMonthTo(e.target.value);
                                        if (e.target.value < archiveMonthFrom) setArchiveMonthFrom(e.target.value);
                                    }}
                                    min={archiveMonthFrom}
                                    className="verify-archive-month-input"
                                    title="To month"
                                />
                            </div>
                            <div className="verify-archive-filter-group">
                                ...
                                <select
                                    ...
                                    className="verify-archive-agent-select"
                                >
                ...
                >
                    ...
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                        ...
                        <div 
                            ...
                            onClick={() => { setIsArchiveView(!isArchiveView); setSelectedAgent(''); }}
