...
const DailyActivityForm = ({
  ...
  const getWeekStart = (dateInput, weekScheduleType = scheduleType) => {
    ...
    const date = typeof dateInput === 'string'
      ...
      : new Date(dateInput);
    date.setHours(0, 0, 0, 0); // Normalize to midnight local to prevent UTC date shift
  ...
  const [internalRange, setInternalRange] = useState("week"); // week, month, ytd
  const [internalDate, setInternalDate] = useState(new Date());

  // When controlled, use external state; otherwise use internal state
  const selectedRange = controlled && externalRange !== undefined ? externalRange : internalRange;
  const currentDate  = controlled && externalDate  !== undefined ? externalDate  : internalDate;
  const setSelectedRange = controlled ? (v) => onRangeChange?.(v)       : setInternalRange;
  const setCurrentDate   = controlled ? (v) => onCurrentDateChange?.(v) : setInternalDate;
  ...
  const [cellWarning, setCellWarning] = useState(null); // { message, top, left }
  const cellWarningTimer = useRef(null);
  const lastEditedCellRef = useRef(null);
  const [showHierarchy, setShowHierarchy] = useState(false);
  const [showMgaTable, setShowMgaTable] = useState(false);
  // When controlled by a parent with a viewScope, team scope forces the MGA table
  const effectiveShowMgaTable = (controlled && viewScope === 'team') ? true : showMgaTable;
  ...
  // Maximum allowed values for daily activity fields
  const FIELD_LIMITS = {
    calls: 1000, appts: 35, sits: 35, sales: 35,
    alp: 50000, refs: 300, refAppt: 35, refSit: 35, refSale: 35, refAlp: 50000
  };
  ...
  const columns = React.useMemo(() => [
    ...
    // Day of week column (second)
    {
      ...
      width: 30,
      Cell: ({ value, row }) => {
        if (row.original.isTargetRow) {
          return <span className="target-row-label">{value}</span>;
        }
    ...
    },
    {
      ...
      width: 40,
      Cell: ({ value, row }) => {
        if (row.original.isTargetRow) return null;
    ...
    },
    {
      ...
      Cell: ({ value, row, isEditing, updateCell }) => {
        ...
        // Target row: show actual period ALP vs target
        if (row.original.isTargetRow) {
          const actual = row.original.actualAlp || 0;
          const target = reportedAlp || 0;
          const fmtAlp = (v) => v >= 1000 ? `$${(v / 1000).toFixed(1)}k` : `$${Math.round(v)}`;
          return (
            <div className="alp-cell-container">
              <div className="alp-actual-value">{fmtAlp(actual)}</div>
              <div className="alp-reported-value">/{fmtAlp(target)}</div>
            </div>
          );
        }
  ...
  }, [dateRows, activityData, actualAlpData, discordSalesData, discordVsManualBreakdown, editedRows, showWeeklyTotals, selectedRange, showMissingDataWarnings, breakdown]);
  ...
  const transformDataForTable = () => {
    ...
    // Prepend target row at top when goals breakdown is available (week or month only)
    if (breakdown?.monthly && selectedRange !== 'ytd') {
      const targets = selectedRange === 'week' ? breakdown.weekly : breakdown.monthly;
      const actualAlp = transformedData
        .filter(r => !r.isWeeklyTotal && !r.isTargetRow)
        .reduce((sum, r) => sum + (parseFloat(r.alp) || 0), 0);
      transformedData.unshift({
        id: 'target-row',
        dayOfWeek: selectedRange === 'week' ? 'Week Target' : 'Month Target',
        displayDate: '',
        calls: targets.calls,
        appts: targets.appts,
        sits: targets.sits,
  ...
  const updateRowClassNames = () => {
    ...
    tableData.forEach(row => {
      ...
      if (row.isTargetRow) {
        classes.push("activity-target-row");
      } else if (row.isWeeklyTotal) {
  ...
  const handleCellUpdate = (id, field, value) => {
    if (id === 'target-row') return;
    const reportDate = id; // id is the reportDate for daily rows

    // Capture the active cell's position for validation popup positioning
    const activeInput = document.activeElement;
    if (activeInput) {
      const td = activeInput.closest('td');
      if (td) {
        lastEditedCellRef.current = td;
      }
    }
  ...
  const handleCellBlur = (id, field, value) => {
    ...
    if (limit && numericValue !== null && numericValue > limit) {
      setEditedRows(prev => ({
        ...prev,
        [id]: {
          ...prev[id],
      ...
      // Show popup below the cell using the position captured during editing
      const td = lastEditedCellRef.current;
      if (td) {
        const rect = td.getBoundingClientRect();
        setCellWarning({
          message: `Max ${limit.toLocaleString()}`,
          top: rect.bottom + 6,
          left: rect.left + (rect.width / 2)
        });
      }
      if (cellWarningTimer.current) clearTimeout(cellWarningTimer.current);
      cellWarningTimer.current = setTimeout(() => setCellWarning(null), 2500);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    // Safety net: clamp any values that exceed max limits
    let hadOverLimit = false;
    Object.keys(correctedEditedRows).forEach(reportDate => {
      const rowEdits = correctedEditedRows[reportDate];
      Object.keys(rowEdits).forEach(field => {
        const limit = FIELD_LIMITS[field];
        if (limit && Number(rowEdits[field]) > limit) {
          correctedEditedRows[reportDate][field] = limit;
          hadOverLimit = true;
        }
      });
    });
    if (hadOverLimit) {
      console.log('[SUBMIT] Some values exceeded maximum limits and were clamped before saving.');
    }
  ...
  return (
    <div className="daily-activity-report">
      ...
      {cellWarning && (
        <div className="cell-warning-popup" style={{ top: cellWarning.top, left: cellWarning.left }}>
          <div className="cell-warning-arrow" />
          {cellWarning.message}
        </div>
      )}
      <div className="padded-content">
        ...
        {/* Navigation and Period Selection - Above Table (hidden when header controls are active) */}
        {!controlled && <div className="controls-container">
        ...
        <ActionBar
          ...
          archivedView={false}
          leftContent={
            selectedRange === 'ytd' ? null : (
              breakdown?.monthly?.alp ? (
                <div className="activity-goal-display">
                  <span className="activity-goal-amount">
                    {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }).format(breakdown.actualMonthAlp || 0)}
                  </span>
                  <span className="activity-goal-label">
                    / {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }).format(breakdown.monthly.alp)}
                  </span>
                  <button
                    className="activity-goal-edit-btn"
                    onClick={() => onGoalEdit?.()}
                ...
                <button
                  className="activity-goal-set-btn"
                  onClick={() => onGoalEdit?.()}
                >
                  Set Monthly Goal
                </button>
        ...
        >
          ...
          <div className="action-buttons-right">
            ...
            <button 
              className={`hierarchy-toggle-button icon-only ${effectiveShowMgaTable ? 'active' : ''}`}
              onClick={() => setShowMgaTable(!showMgaTable)}
              title={effectiveShowMgaTable ? 'Hide MGA Teams' : 'Show MGA Teams'}
            >
              <svg 
                width="18" 
                height="18" 
              ...
              return (
                <>
                  <DataTable
                    ...
                    isRowExpandable={(row) => !row.isTargetRow && !row.isWeeklyTotal}
