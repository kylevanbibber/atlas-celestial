...
const PotentialVIPs = ({ searchQuery = '', filters = {}, onFilterOptions }) => {
  ...
  const transformDataForExport = (data) => {
    return data.map(item => ({
      ...
      weekly_recap_net: typeof item.weeklyRecapNet === 'number' ? item.weeklyRecapNet : parseFloat(item.weeklyRecapNet || 0),
  ...
  const columns = useMemo(() => [
    ...
    {
      Header: 'Weekly Recap Net',
      accessor: 'weeklyRecapNet',
      filterable: false,
      Cell: ({ value }) => (
        <span className="gross-amount">
          {usdFormatter.format(Number(value) || 0)}
