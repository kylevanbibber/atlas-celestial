...
const APPLICANT_STEP_LABELS = {
  'Careers Form': 'Applicant',
  'No Answer - Career Form': 'No Answer',
  'Callback - Career Form': 'Callback',
  'Final': 'Booked Final',
...
const PipelineKanban = ({
  ...
  stageChecklistItems, recruitProgress,
  ...
  const [justCheckedInIds, setJustCheckedInIds] = useState(new Set());
  ...
  const applicantRecruits = useMemo(() => {
    let appRecruits = filteredRecruits.filter(r => APPLICANT_STEPS.includes(r.step));

    // Apply global sort
    appRecruits.sort((a, b) => {
      if (globalSort === 'a-z') {
        return `${a.recruit_first} ${a.recruit_last}`.toLowerCase().localeCompare(`${b.recruit_first} ${b.recruit_last}`.toLowerCase());
      } else if (globalSort === 'z-a') {
        return `${b.recruit_first} ${b.recruit_last}`.toLowerCase().localeCompare(`${a.recruit_first} ${a.recruit_last}`.toLowerCase());
      } else if (globalSort === 'newest-added') {
        return (parseDateString(b.date_added) || new Date(0)) - (parseDateString(a.date_added) || new Date(0));
      }
      // Default: oldest-added
      return (parseDateString(a.date_added) || new Date(0)) - (parseDateString(b.date_added) || new Date(0));
    });

    return appRecruits;
  }, [filteredRecruits, globalSort]);
  ...
  const handleStepChange = async (recruitId, newStep) => {
    try {
      setRecruits(prev => prev.map(r => r.id === recruitId ? { ...r, step: newStep } : r));
      await api.put(`/recruitment/recruits/${recruitId}/step`, { step: newStep });
    } catch (error) {
  ...
  const handleCardClick = (recruit) => {
    const isApplicant = APPLICANT_STEPS.includes(recruit.step);
    const detailsData = isApplicant
      ? { ...recruit, __isApplicantDetails: true }
      : { recruit, stages, __isPipelineChecklist: true };
    ...
    <div className="pipeline-kanban-container">
      ...
      {stages.length > 0 && (
        <div className="pipeline-kanban-board">
          {/* Applicants Column */}
          <div className="pipeline-kanban-column">
            <div
              className="pipeline-kanban-column-header"
              style={{ borderTopColor: '#3498db' }}
            >
              <h3>Applicants</h3>
              <span className="pipeline-kanban-count">{applicantRecruits.length}</span>
            </div>
            <div className="pipeline-kanban-column-content">
              {applicantRecruits.map((recruit) => (
                <div
                  key={recruit.id}
                ...
                >
                  ...
                  <select
                    className="kanban-applicant-step-select"
                    value={recruit.step}
                    onClick={e => e.stopPropagation()}
                    onChange={(e) => {
                      e.stopPropagation();
                      handleStepChange(recruit.id, e.target.value);
                    }}
                    style={{
                      backgroundColor: APPLICANT_STEP_COLORS[recruit.step] || '#3498db',
                      color: '#fff',
                      border: 'none',
                      padding: '2px 8px',
                      borderRadius: '10px',
                  ...
                  >
                    {APPLICANT_STEPS.map(step => (
                      <option key={step} value={step} style={{ backgroundColor: '#fff', color: '#333' }}>
                        {APPLICANT_STEP_LABELS[step] || step}
                      </option>
                    ))}
                  </select>
                  ...
                  {recruit.phone && (
                    <div className="kanban-card-detail">
                      <FiPhone size={12} />
                      <a href={`tel:${recruit.phone}`} onClick={e => e.stopPropagation()} className="kanban-card-link">{recruit.phone}</a>
                    </div>
                  )}
                  ...
                  {recruit.email && (
                    <div className="kanban-card-detail">
                      <FiMail size={12} />
                      <a href={`mailto:${recruit.email}`} onClick={e => e.stopPropagation()} className="kanban-card-link">{recruit.email}</a>
                    </div>
                  )}
                  ...
                  {recruit.referral_source && (
                    <div className="kanban-card-detail" style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>
                      Source: {recruit.referral_source}
                    </div>
                  )}
              ...
              {applicantRecruits.length === 0 && !loading && (
                ...
                {stageRecruits.map((recruit) => (
                  ...
                  >
                              ...
                              {recruit.xcel_progress_pct !== null && recruit.xcel_progress_pct !== undefined && (
                                <div className={`kanban-xcel-progress ${
                                  recruit.xcel_progress_pct >= 75 ? 'xcel-high' :
                                  recruit.xcel_progress_pct >= 50 ? 'xcel-mid' :
                                  recruit.xcel_progress_pct >= 25 ? 'xcel-low' : 'xcel-minimal'
                                }`}>
      ...
      {showRightDetails && rightDetailsData && (
        <RightDetails
          ...
          onStepChange={(id, newStep) => handleStepChange(id, newStep)}
        />
      )}
