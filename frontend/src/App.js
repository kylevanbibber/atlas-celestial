...
import TextCampaigns from "./pages/TextCampaigns";
...
import PromotionTracking from "./components/promotion-tracking/PromotionTracking";
...
function AppContent() {
  ...
  // Check if user is an app admin (teamRole='app' in activeusers)
  const isAppAdmin = user?.teamRole === 'app';
  ...
  React.useEffect(() => {
    ...
    if (isAuthenticated && isAuthPage) {
      // All users go to dashboard (which renders appropriate content based on role)
    ...
    <div className={`app-container ${isEmbedded ? 'embedded-mode' : ''}`}>
      {!isEmbedded && (
        <>
          ...
          {user && user?.clname !== 'SGA' && user?.teamRole !== 'app' && (
            <DailyActivityReminder user={user} />
          )}
        </>
      )}
      <div className="main-content">
        <div className="page-content">
          <Routes>
            ...
            />
            <Route
              path="/recruiting/onboarding-preview/:recruitId"
              element={
                <ProtectedRoute>
                  <OnboardingPreview />
                </ProtectedRoute>
            ...
            />
            <Route
              path="/auth/outlook-calendar/callback"
              element={
                <ProtectedRoute>
