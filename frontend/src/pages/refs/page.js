...
import { FiCalendar, FiFilter, FiSearch, FiPhone } from 'react-icons/fi';
import RefDetails from "../../components/utils/RefDetails";
import RefScheduler from "../../components/utils/RefScheduler";
import RefDialer from "../../components/utils/RefDialer";
...
const RefsPage = () => {
  ...
  const [schedulerOpen, setSchedulerOpen] = useState(null); // { refId, refName, assignedTo, value, position }
  const [showDialer, setShowDialer] = useState(false);
  const [dialerCurrentRefId, setDialerCurrentRefId] = useState(null);
  const [tallyStatus, setTallyStatus] = useState(null); // { isLinked, subscription }
  const [showTallyPrompt, setShowTallyPrompt] = useState(false);
  ...
  // Check Tally link + subscription status
  useEffect(() => {
    const checkTally = async () => {
      try {
        const res = await api.get('/account/tally/status');
        if (res.data?.success) {
          setTallyStatus(res.data);
        }
      } catch {
        // Not linked or error — dialer stays hidden
      }
    };
    checkTally();
  }, []);

  const tallyDialerEnabled = tallyStatus?.isLinked &&
    ['active', 'trialing'].includes(tallyStatus?.subscription?.status);
  ...
  const fetchRefs = async () => {
    try {
      ...
      const params = { archive: archivedView ? 1 : 0 };

      if (canSeeAllRefs) {
        params.filter_mode = 'all';
      } else if (isAgent) {
        params.filter_mode = 'own';
        params.user_id = userId;
      } else if (hasHierarchyAccess && allowedIds.length > 0) {
        params.filter_mode = 'hierarchy';
        params.allowed_ids = allowedIds.join(',');
      } else {
        params.filter_mode = 'own';
        params.user_id = userId;
      }

      const response = await api.get('/refs', { params });

      // Sort by date_created descending (newest first) by default
  ...
  const columns = [
        ...
        if (value) {
          if (typeof value === 'string' && value.match(/^\d{1,2}\/\d{1,2}\/\d{2} \d{1,2}:\d{2} [AP]M$/)) {
            ...
            if (!isNaN(d.getTime())) {
              ...
              const h12 = h % 12 || 12;
              display = `${d.getMonth() + 1}/${d.getDate()}/${d.getFullYear().toString().slice(-2)} ${h12}:${String(d.getMinutes()).padStart(2, '0')} ${ampm}`;
            }
          }
        }
        return (
          <div style={{ cursor: 'pointer', minHeight: '1.5em' }}
            onClick={(e) => {
              e.stopPropagation();
              const rect = e.currentTarget.getBoundingClientRect();
              setSchedulerOpen(prev => prev?.refId === row.original.id ? null : {
                refId: row.original.id,
                refName: row.original.name,
          ...
          >
            <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
              {display || <span style={{ color: '#999', fontSize: '12px' }}>Click to schedule</span>}
              <FiCalendar size={12} style={{ color: '#666', flexShrink: 0 }} />
            </span>
          </div>
        );
      }
    ...
    },
    {
      ...
      dropdownOptionLabels: { "": "Select State" },
      dropdownBackgroundColor: (value) => value ? "#f0f8ff" : "#ffffff",
      Cell: ({ value }) => {
        if (!value) {
          return (
            <span style={{ color: '#999', fontSize: '12px' }}>Select State</span>
          );
        }
    ...
    },
    {
      ...
      filterKey: "assigned_to_display",
      chipDropdown: true,
      width: 150,
      chipDropdownOptions: users,
      chipDropdownValueField: "id",
      chipDropdownDisplay: (user) => user.first_name,
  ...
  const handleCellUpdate = async (id, field, value) => {
    try {
      // Normalize id to number for consistent comparison (DataTable passes string keys)
      const numId = typeof id === 'string' ? parseInt(id, 10) : id;
      // Get the current ref to check previous values
      const currentRef = tableData.find(row => row.id === numId);
      ...
      setTableData(prevTableData => {
        const updatedData = prevTableData.map((row) =>
          row.id === numId ? { ...row, ...updateData } : row
        );
      ...
      await api.put(`/refs/${numId}`, apiUpdateData);
  ...
  const handleMassStatusChange = async (newStatus, selectedIds) => {
    // DataTable's handleHeaderStatusChange already calls onCellUpdate per row
    // which handles both the API call and optimistic update, so we only need
    // to ensure our tableData state is in sync
    const selectedIdNums = new Set(selectedIds.map(id => parseInt(id, 10)));
    setTableData(prev =>
      prev.map(row =>
        selectedIdNums.has(row.id) ? { ...row, status: newStatus } : row
      )
    );
  };
  ...
  const handleImportData = async (importedData) => {
    try {
      ...
      if (Array.isArray(importedData)) {
        dataToSend = importedData.map(item => {
          if (typeof item === 'object' && item !== null) {
            const row = { ...item };
            // Set created_by and assigned_to to current user if not provided
            const userId = user?.userId || user?.id;
      ...
      await api.post('/refs/import', dataToSend);
      fetchRefs();
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    if (!window.confirm(`Are you sure you want to delete ${selectedRows.length} ref(s)? This cannot be undone.`)) return;
    try {
      const ids = selectedRows.map(id => parseInt(id)).filter(id => !isNaN(id));
      if (ids.length === 0) return;
      await api.post('/refs/batch-delete', { ids });
  ...
  return (
    <div>
          ...
          <div style={{ padding: "20px" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "20px" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                <button
                  onClick={() => tallyDialerEnabled ? setShowDialer(!showDialer) : setShowTallyPrompt(true)}
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: '6px',
                    padding: '8px 14px',
                    background: showDialer ? '#00558c' : !tallyDialerEnabled ? 'var(--bg-secondary, #f0f0f0)' : 'var(--bg-secondary, #f0f0f0)',
                    color: showDialer ? '#fff' : !tallyDialerEnabled ? 'var(--text-secondary, #999)' : 'var(--text-primary, #333)',
                    border: 'none',
                    borderRadius: '6px',
                    cursor: tallyDialerEnabled ? 'pointer' : 'default',
                    fontSize: '13px',
                    fontWeight: 500,
                    opacity: tallyDialerEnabled ? 1 : 0.5,
                    transition: 'all 0.15s'
                  }}
                  title={tallyDialerEnabled ? 'Open Dialer' : 'Tally subscription required'}
                >
                  <FiPhone size={14} />
                  <span>Dialer</span>
                </button>
            ...
            {showDialer && (
              <RefDialer
                refs={filteredData}
                onClose={() => { setShowDialer(false); setDialerCurrentRefId(null); }}
                onUpdateStatus={handleCellUpdate}
                onCurrentRefChange={setDialerCurrentRefId}
            ...
            <DataTable
              ...
              rowClassNames={dialerCurrentRefId ? { [dialerCurrentRefId]: 'dialer-active-row' } : {}}
      ...
      {schedulerOpen && (
        <RefScheduler
          value={schedulerOpen.value}
          assignedTo={schedulerOpen.assignedTo}
          refName={schedulerOpen.refName}
          position={schedulerOpen.position}
          onSchedule={(isoValue) => {
            const refId = schedulerOpen.refId;
            setSchedulerOpen(null);
            handleCellUpdate(refId, 'scheduled', isoValue);
          }}
      ...
      {showTallyPrompt && (
        ...
        }} onClick={() => setShowTallyPrompt(false)}>
          <div style={{
            background: 'var(--card-bg, #fff)', borderRadius: '12px', padding: '32px',
            maxWidth: '400px', width: '90%', textAlign: 'center',
            boxShadow: '0 8px 32px rgba(0,0,0,0.2)'
          }} onClick={e => e.stopPropagation()}>
            <FiPhone size={32} style={{ color: '#00558c', marginBottom: '12px' }} />
            <h3 style={{ margin: '0 0 8px', fontSize: '18px', color: 'var(--text-primary, #333)' }}>
              Tally Subscription Required
            </h3>
            <p style={{ margin: '0 0 20px', fontSize: '14px', color: 'var(--text-secondary, #666)', lineHeight: 1.5 }}>
              {tallyStatus?.isLinked
                ? 'Your Tally account needs an active subscription to use the dialer.'
                : 'Link your Tally account and subscribe to a plan to use the built-in dialer.'}
            </p>
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
              <button
                onClick={() => setShowTallyPrompt(false)}
                style={{
                  padding: '10px 20px', borderRadius: '8px', border: '1px solid var(--border-color, #ddd)',
                  background: 'var(--card-bg, #fff)', color: 'var(--text-primary, #333)',
                  cursor: 'pointer', fontSize: '14px'
                }}
              >Cancel</button>
              <button
                onClick={() => { setShowTallyPrompt(false); navigate('/utilities?section=tally'); }}
                style={{
                  padding: '10px 20px', borderRadius: '8px', border: 'none',
                  background: '#00558c', color: '#fff', cursor: 'pointer',
