...
const LeaderboardPage = () => {
    ...
    // Includes dedup/aggregation logic (ported from TeamDashboard.js) to handle:
    // 1. Multiple CTLNO codes per user (sum them)
    // 2. Multiple reportdates within 3-day window (Arias Main + New York reports)
    // 3. CL_Name paired rows for MGA vs RGA views
    // 4. Exact database duplicate rows
    const processLeaderboardData = (data, key) => {
        // Step 1: Filter by experience
        const experienceFiltered = data.filter(filterByExperience);

        // Step 2: Normalize the date field (reportdate for Weekly_ALP, month for Monthly_ALP)
        const isMonthlyData = experienceFiltered.length > 0 && !experienceFiltered[0].reportdate && experienceFiltered[0].month;
        const normalizedData = experienceFiltered.map(item => ({
            ...item,
            reportdate: item.reportdate || item.month
        }));

        // Step 3: Find the most recent reportdate for each user
        ...
        // Step 4: Aggregate rows by LagnName with dedup
        const alpDataMap = new Map();
        const seenRows = new Map();

        normalizedData.forEach(item => {
            if (!item.LagnName) return;
            ...
            // For Weekly_ALP, skip rows more than 3 days before the user's max reportdate
            // For Monthly_ALP, include all rows from the month
            if (!isMonthlyData && maxDate) {
                const daysDiff = (maxDate - itemDate) / (1000 * 60 * 60 * 24);
                if (daysDiff > 3) return;
            }

            // For MGA/RGA views, apply CL_Name filtering
            // If 2 rows exist for same (LagnName, reportdate, REPORT): CL_Name='MGA' is MGA, CL_Name='' is RGA
            // If 1 row exists: include it in both views
            if (key === 'mga' || key === 'rga') {
                ...
                if (pairedRows.length >= 2 && hasMgaRow && hasBlankRow) {
                    if (key === 'mga' && item.CL_Name !== 'MGA') return;
                    if (key === 'rga' && item.CL_Name === 'MGA') return;
                }
            }

            // Check for exact duplicate rows (same user, date, report, and ALP values)
            const lvl1Net = parseFloat(item.LVL_1_NET) || 0;
            const lvl3Net = parseFloat(item.LVL_3_NET) || 0;
            const dedupeKey = `${item.LagnName}|${item.reportdate}|${item.REPORT}|${lvl1Net}|${lvl3Net}`;

            if (seenRows.has(dedupeKey)) return;
            seenRows.set(dedupeKey, true);

            // Aggregate ALP values across rows for the same user
            const existing = alpDataMap.get(item.LagnName);
            if (existing) {
                existing.LVL_1_NET = (parseFloat(existing.LVL_1_NET) || 0) + (parseFloat(item.LVL_1_NET) || 0);
                existing.LVL_1_GROSS = (parseFloat(existing.LVL_1_GROSS) || 0) + (parseFloat(item.LVL_1_GROSS) || 0);
                existing.LVL_2_NET = (parseFloat(existing.LVL_2_NET) || 0) + (parseFloat(item.LVL_2_NET) || 0);
                existing.LVL_2_GROSS = (parseFloat(existing.LVL_2_GROSS) || 0) + (parseFloat(item.LVL_2_GROSS) || 0);
                existing.LVL_2_F6_NET = (parseFloat(existing.LVL_2_F6_NET) || 0) + (parseFloat(item.LVL_2_F6_NET) || 0);
                existing.LVL_2_F6_GROSS = (parseFloat(existing.LVL_2_F6_GROSS) || 0) + (parseFloat(item.LVL_2_F6_GROSS) || 0);
                existing.LVL_3_NET = (parseFloat(existing.LVL_3_NET) || 0) + (parseFloat(item.LVL_3_NET) || 0);
                existing.LVL_3_GROSS = (parseFloat(existing.LVL_3_GROSS) || 0) + (parseFloat(item.LVL_3_GROSS) || 0);
                existing.LVL_3_F6_NET = (parseFloat(existing.LVL_3_F6_NET) || 0) + (parseFloat(item.LVL_3_F6_NET) || 0);
                existing.LVL_3_F6_GROSS = (parseFloat(existing.LVL_3_F6_GROSS) || 0) + (parseFloat(item.LVL_3_F6_GROSS) || 0);
                ...
                alpDataMap.set(item.LagnName, {
                    ...item,
                    LVL_1_NET: parseFloat(item.LVL_1_NET) || 0,
                    LVL_1_GROSS: parseFloat(item.LVL_1_GROSS) || 0,
                    LVL_2_NET: parseFloat(item.LVL_2_NET) || 0,
                    LVL_2_GROSS: parseFloat(item.LVL_2_GROSS) || 0,
                    LVL_2_F6_NET: parseFloat(item.LVL_2_F6_NET) || 0,
                    LVL_2_F6_GROSS: parseFloat(item.LVL_2_F6_GROSS) || 0,
                    LVL_3_NET: parseFloat(item.LVL_3_NET) || 0,
                    LVL_3_GROSS: parseFloat(item.LVL_3_GROSS) || 0,
                    LVL_3_F6_NET: parseFloat(item.LVL_3_F6_NET) || 0,
                    LVL_3_F6_GROSS: parseFloat(item.LVL_3_F6_GROSS) || 0,
                });
            }
        });

        // Step 5: Map aggregated data to leaderboard format
        return Array.from(alpDataMap.values())
            .map((row, index) => {
                let alpValue;
