...
const AccountSetupPage = ({ onBackToLogin, lagnname, email: defaultEmail, phone: defaultPhone, id, needsPasswordChange, isRedeemed }) => {
  const { user } = useAuth();
...
const [modalContent, setModalContent] = useState({
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate password fields if password change is required
    ...
    // For redeemed users who only need to set a password, skip the full registration flow
    if (isRedeemed && needsPasswordChange) {
      try {
        ...
        if (createData.success) {
          ...
          alert(createData.message || 'Failed to update password. Please try again.');
        }
      } catch (error) {
    ...
    // Full registration flow for non-redeemed users
    ...
    try {
      ...
      if (data.success) {
        ...
        const { data: createData } = await api.post('/auth/handleUserInfo', {
          ...
          lagnname: lagnname,
          decision: 'new',
          ...(needsPasswordChange && newPassword ? { newPassword } : {}),
        });
  ...
  // Simplified view for redeemed users who only need to set their password
  if (isRedeemed && needsPasswordChange) {
    return (
      <div className="login-container">
        <div className="login-card">
          <div className="left-side">
            <h2>Welcome back, {formData.screenName}.</h2>
            <p>Your account requires a password update before you can continue.</p>
          </div>
          <div className="right-side">
            <h2>Set Your Password</h2>
            <a href="#" onClick={onBackToLogin} className="account-setup-back-link" style={{ marginBottom: '16px', display: 'inline-block' }}>
              &larr; Back to Login
            </a>
            <form onSubmit={handleSubmit} className="account-setup-register-form">
              ...
              <div className="account-setup-form-field">
                <label className="account-setup-form-label">*New Password</label>
                <input
                  type="password"
                  value={newPassword}
              ...
              <div className="account-setup-form-field">
                <label className="account-setup-form-label">*Confirm Password</label>
                <input
                  type="password"
              ...
              {passwordError && <span className="account-setup-error-message">{passwordError}</span>}
  ...
  // Full registration view for non-redeemed users
  return (
      <div className="login-container">
    ...
    <div className="modal-content">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="account-setup-register-form">
          ...
          {/* Password fields - shown when user needs to set their password (first login / default password) */}
          {needsPasswordChange && (
            <>
