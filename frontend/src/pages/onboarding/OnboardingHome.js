...
const OnboardingHome = ({ previewPipelineId }) => {
  const navigate = useNavigate();
  const pipelineId = previewPipelineId || localStorage.getItem('onboardingPipelineId');
  const email = localStorage.getItem('onboardingEmail');
  const isPreview = !!previewPipelineId;
  ...
  useEffect(() => {
    const run = async () => {
      ...
      setError('');
      try {
        const [itemsResp, progResp, stagesResp, summaryResp, xcelResp] = await Promise.all([
        ...
        // Fetch attachments separately — non-critical, may 403 for managers in preview mode
        let atts = [];
        try {
          const attResp = await api.get(`/pipeline-attachments/recruit/${pipelineId}`);
          atts = attResp.data?.data || attResp.data?.attachments || [];
        } catch (_) {}
      ...
      {isPreview && (
        <div style={{
          ...
          <span style={{ fontSize: 14, fontWeight: 600 }}>
            Preview Mode — Viewing onboarding as {recruitSummary ? formatCodedToName(recruitSummary.first_name ? `${recruitSummary.last_name} ${recruitSummary.first_name}` : recruitSummary.name) : 'recruit'}
          </span>
        </div>
      )}
      {/* Page Content */}
        ...
        {recruitSummary && (
          <div style={{
            ...
            <div style={{ color: 'var(--text-primary)' }}>
              <strong>Reach out to your leader</strong> when you complete a step or if you need help!
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 12 }}>
              ...
              <div>
                <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>Leader Name</div>
            ...
            <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start', flexWrap: 'wrap' }}>
              <div>
                <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>Leader Email</div>
              ...
              <div>
                <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>Leader Phone</div>
                                      ...
                                      {(!recruitSummary?.course || !recruitSummary?.expected_complete_date || courseEditing || expectedDateEditing) ? (
                                        ...
                                        {!expectedDateEditing ? (
                                          ...
                                          <>
                                            ...
                                            <button
                                              disabled={isPreview || expectedDateSaving || courseSaving}
                                              onClick={async (e) => {
                                                e.stopPropagation();
                                                if (isPreview) return;
                                                try {
      ...
      {showTutorial && (() => {
        const tutorialSteps = [
          ...
          },
          {
            ...
            description: "This is your leader's contact information. Reach out to them anytime you need help or have questions!",
          ...
          {
            title: "Mark Complete",
            description: "When you finish a task, click the checkbox on the right. This automatically notifies your leader that you've completed the step!",
            target: '.checklist-checkbox',
            position: 'left'
          },
          {
            title: "You're All Set!",
            description: "You're ready to start your onboarding journey. Remember, your leader is here to help if you need anything!",
            target: null,
            position: 'center'
          }
        ];
