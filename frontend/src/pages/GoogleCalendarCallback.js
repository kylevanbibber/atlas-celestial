  ...
  useEffect(() => {
    const handleCallback = async () => {
      try {
        ...
        const response = await api.post('/account/google-calendar/oauth/callback', { code, state });
        ...
        if (response.data.success) {
          ...
          setStatus('error');
          setMessage(response.data.message || 'Failed to connect Google Calendar');
          setTimeout(() => navigate('/utilities?section=account'), 3000);
        }
      } catch (error) {
        console.error('Error in Google Calendar OAuth callback:', error);
        setStatus('error');
        setMessage('An error occurred while connecting your Google Calendar');
        setTimeout(() => navigate('/utilities?section=account'), 3000);
      }
    };
