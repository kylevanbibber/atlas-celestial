...
export const calculateDateRange = (dateValue, type) => {
  ...
  if (type === 'week') {
    ...
    return {
      start_date: formatLocal(monday),
      end_date: formatLocal(sunday),
      type: 'week'
    };
  } else if (type === 'month') {
    ...
    return {
      start_date: formatLocal(monthStart),
      end_date: formatLocal(monthEnd),
      type: 'month'
    };
  } else if (type === 'year') {
    ...
    return {
      start_date: formatLocal(yearStart),
      end_date: formatLocal(yearEnd),
      type: 'year'
    };
  }
