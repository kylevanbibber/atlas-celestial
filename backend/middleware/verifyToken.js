...
// Normalize lagnname: trim and collapse internal whitespace
function trimName(val) {
  return val ? val.replace(/\s+/g, ' ').trim() : val;
}
      ...
      try {
        ...
        if (!userResults || userResults.length === 0) {
          ...
          req.user = {
            ...
            lagnname: trimName(decoded.lagnname),
        ...
        req.user = {
          userId: impersonatedUserId,
          id: impersonatedUserId,
          lagnname: trimName(impersonatedUser.lagnname),
          clname: impersonatedUser.clname,
          Role: impersonatedUser.Role,
          _isImpersonating: true,
          _originalAdminId: decoded.id || decoded.userId,
          _originalAdminName: trimName(decoded.lagnname)
        };
