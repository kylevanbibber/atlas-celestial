...
router.post('/update-more-data', verifyToken, async (req, res) => {
    try {
        ...
        const checkQuery = 'SELECT * FROM amore_data WHERE TRIM(MGA) = TRIM(?) AND MORE_Date = ?';
        ...
        if (existingRecords.length === 0) {
            ...
            const defaultValues = {
                ...
                Finals_Set: 0,
                Finals_Show: 0,
                // New recruiting breakdown fields (safe defaults; DB also has defaults)
                PR_Final_Set: 0,
                PR_Final_Show: 0,
