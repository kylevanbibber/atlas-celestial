...
// Dashboard summary: per-agent refs collected & ref sales for a date range
  ...
  try {
    ...
    const result = await query(`
      SELECT
        agent_id,
        lagnname,
        COUNT(*) AS refs_collected,
        COUNT(CASE WHEN true_ref = 'Y' THEN 1 END) AS ref_sales
      FROM refvalidation
      WHERE DATE(created_at) >= ? AND DATE(created_at) <= ?
        AND agent_id IN (${placeholders})
        AND agent_id IS NOT NULL
      GROUP BY agent_id, lagnname
      ORDER BY refs_collected DESC
    `, [startDate, endDate, ...ids]);

    res.json({ success: true, data: result });
  } catch (error) {
    console.error("Error fetching refvalidation dashboard summary:", error);
    res.status(500).json({ success: false, message: error.message });
  }
});
