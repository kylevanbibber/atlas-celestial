...
const FIELD_LIMITS = {
    ...
    HC_Appt: 35, HC_Sit: 35, HC_Sale: 35, HC_ALP: 50000,
    POS_Appt: 35, POS_Sit: 35, POS_Sale: 35, POS_ALP: 50000,
    Vendor_Appt: 35, Vendor_Sit: 35, Vendor_Sale: 35, Vendor_ALP: 50000
};

function validateActivityFields(data) {
    const errors = [];
    for (const [field, max] of Object.entries(FIELD_LIMITS)) {
        if (data[field] !== undefined && data[field] !== null && Number(data[field]) > max) {
...
router.post("/submit", async (req, res) => {
    ...
    const validationErrors = validateActivityFields({
        HC_Appt, HC_Sit, HC_Sale, HC_ALP, POS_Appt, POS_Sit, POS_Sale, POS_ALP,
        refAppt, refSit, refSale, refAlp, Vendor_Appt, Vendor_Sit, Vendor_Sale, Vendor_ALP,
        calls, appts, sits, sales, alp, refs
    });
    ...
    try {
        const checkQuery = `SELECT * FROM Daily_Activity WHERE TRIM(agent) = TRIM(?) AND reportDate = ?`;
...
router.post("/update", verifyToken, async (req, res) => {
    // For admin impersonation: prefer body userId over authenticated user's ID
    // This allows admins to submit activity on behalf of other users
    const userId = req.body.userId || req.user?.id;
    const { updates } = req.body;
    
    const isImpersonating = req.body.userId && req.body.userId !== req.user?.id;

    console.log('[dailyActivity/update] Received request:', {
        userId,
        authenticatedUserId: req.user?.id,
        bodyUserId: req.body.userId,
        isImpersonating,
        updates: JSON.stringify(updates, null, 2),
        updateDates: updates ? Object.keys(updates) : [],
    });

    if (!userId || !updates || Object.keys(updates).length === 0) {
        console.log('[dailyActivity/update] Validation failed:', { userId, hasUpdates: !!updates, updatesCount: updates ? Object.keys(updates).length : 0 });
        return res.status(400).json({ success: false, message: 'Missing userId or updates' });
    }
    
    // Security check: Only allow impersonation if user is an admin
    if (isImpersonating) {
        // Check if the authenticated user has admin privileges
        // Match AuthContext.js admin checks (lines 1244, 1262)
        const isAdmin = req.user?.Role === 'Admin' ||  // Capital R, Capital A
                       req.user?.permissions === 'Admin' || 
                       req.user?.role === 'admin' ||  // lowercase fallback
                       req.user?.isAdmin === true || 
                       req.user?.teamRole === 'app' ||  // App team role
                       req.user?.clname === 'app';
        
        if (!isAdmin) {
            console.log('[dailyActivity/update] ❌ Unauthorized impersonation attempt:', {
                authenticatedUserId: req.user?.id,
                attemptedUserId: req.body.userId,
                'req.user.Role': req.user?.Role,
                'req.user.role': req.user?.role,
                'req.user.permissions': req.user?.permissions,
                'req.user.teamRole': req.user?.teamRole,
                'req.user.clname': req.user?.clname,
                'req.user.isAdmin': req.user?.isAdmin
            });
            return res.status(403).json({ 
                success: false, 
                message: 'Unauthorized: Cannot submit activity for another user' 
            });
        }
        
        console.log('[dailyActivity/update] ✓ Admin impersonation authorized:', {
            adminUserId: req.user?.id,
            impersonatedUserId: req.body.userId
        });
    }
    ...
    try {
        ...
        console.log(`[dailyActivity/update] Fetching user data for userId=${userId}`);
        ...
        for (const reportDate of Object.keys(updates)) {
            const validationErrors = validateActivityFields(updates[reportDate]);
            if (validationErrors.length > 0) {
                return res.status(400).json({ success: false, message: `Validation failed for ${reportDate}: ${validationErrors.join(', ')}` });
            }
        }
        ...
        const promises = Object.keys(updates).map(async (reportDate) => {
            ...
            console.log(`[dailyActivity/update] Processing date ${reportDate}:`, data);
            ...
            if (existingRecord && existingRecord.length > 0) {
                console.log(`[dailyActivity/update] Found existing record for ${reportDate}, updating...`);
                ...
                console.log(`[dailyActivity/update] Fields to update:`, fieldsToUpdate, 'Values:', valuesToUpdate);
                ...
                if (fieldsToUpdate.length > 0) {
                    ...
                    console.log(`[dailyActivity/update] Executing UPDATE query:`, updateQuery);
                    const result = await query(updateQuery, valuesToUpdate);
                    console.log(`[dailyActivity/update] UPDATE result:`, result);
                    return result;
                }
            } else {
                console.log(`[dailyActivity/update] No existing record for ${reportDate}, inserting new...`);
                ...
                console.log(`[dailyActivity/update] INSERT values:`, insertValues);
                const result = await query(insertQuery, insertValues);
                console.log(`[dailyActivity/update] INSERT result:`, result);
                return result;
            }
        });
        ...
        console.log('[dailyActivity/update] ✓ All updates completed successfully');
        res.status(200).json({ success: true, message: 'Daily activity updated successfully' });
    } catch (err) {
        console.error('[dailyActivity/update] ❌ Error updating daily activity data:', err);
...
router.put("/", async (req, res) => {
    ...
    // Validate field limits
    const validationErrors = validateActivityFields(updateFields);
    if (validationErrors.length > 0) {
        return res.status(400).json({ success: false, message: `Validation failed: ${validationErrors.join(', ')}` });
    }
    ...
    try {
        ...
        // Check if record exists
        const existingQuery = `SELECT * FROM Daily_Activity WHERE TRIM(agent) = TRIM(?) AND reportDate = ?`;
        ...
        if (existingRecord && existingRecord.length > 0) {
            ...
            const updateQuery = `UPDATE Daily_Activity SET ${setClause} WHERE reportDate = ? AND TRIM(agent) = TRIM(?)`;
...
router.get("/agent/:agent/:date", async (req, res) => {
    ...
    try {
        const queryStr = `SELECT * FROM Daily_Activity WHERE TRIM(agent) = TRIM(?) AND reportDate = ?`;
