...
router.post('/password-assist/send-to-mga', async (req, res) => {
  try {
    ...
    // Find MGA email from activeusers
    const mgaRow = await query(
      `SELECT lagnname, COALESCE(email, '') AS email
       FROM activeusers
       WHERE lagnname = ? LIMIT 1`,
      [mga]
    );
...
/* ----------------------
   Toggle Account Active Status Route
   Toggles activeusers.Active ('y'/'n') — controls whether a user can log in at all.
   Only staff (SA/GA/MGA/RGA/SGA) and admins can use this.
------------------------- */
router.post("/toggleAccountActive", verifyToken, async (req, res) => {
  try {
    const { userId, currentStatus } = req.body;
    ...
    // Verify requesting user is staff or admin
    const requestingUser = await query(
      'SELECT Role, teamRole, clname FROM activeusers WHERE id = ?',
      [req.userId]
    );
    ...
    const u = requestingUser[0];
    const isStaff = ['Admin', 'SuperAdmin', 'admin', 'superadmin'].includes(u.Role) ||
                    u.teamRole === 'app' ||
                    ['SA', 'GA', 'MGA', 'RGA', 'SGA'].includes(u.clname);
    ...
    const userQuery = await query(
      'SELECT id, lagnname, Active FROM activeusers WHERE id = ?',
      [userId]
    );
    ...
    const result = await query(
      'UPDATE activeusers SET Active = ? WHERE id = ?',
      [newStatus, userId]
    );
    ...
    console.log(`✅ Account Active toggled for user ${userQuery[0].lagnname} (id: ${userId}) to ${newStatus} by user ${req.userId}`);

    res.json({
      success: true,
      message: `Account status updated to ${newStatus === 'y' ? 'active' : 'inactive'}`,
      newStatus
    });
  } catch (error) {
    console.error('❌ Error in toggleAccountActive:', error);
...
router.get("/profile", verifyToken, async (req, res) => {
  try {
    ...
    let user = await query(
      'SELECT id, lagnname, email, phone, profpic, header_pic, clname, Role, screen_name, esid, mga, agtnum, bio, teamRole, discord_id FROM activeusers WHERE id = ? AND Active = "y"',
      [userId]
    );
...
router.post("/newlogin", async (req, res) => {
  ...
  try {
    ...
    if (matchingRegularUsers.length > 0) {
      ...
      // User exists in activeusers but password doesn't match — stop here, do NOT fall through to admin_logins
      if (!validRegularUser) {
        return res.status(401).send({ 
          success: false, 
          message: "Incorrect password. Please try again.",
          errorType: "INVALID_PASSWORD",
          details: "The password provided does not match your account."
        });
      }

      // If password is still "default", force user to set a new password via the register/setup page
        if (!validRegularUser.Password || validRegularUser.Password === "default") {
          return res.status(200).send({
            success: true,
            message: "Please complete account setup",
            needsPasswordChange: true,
            redeemed: parseInt(validRegularUser.redeemed) === 1 ? 1 : 0,
        ...
        if (parseInt(validRegularUser.redeemed) === 1) {
          ...
          if (validRegularUser.clname === "RGA" || validRegularUser.clname === "MGA") {
            const mgaRgaData = await query("SELECT * FROM MGAs WHERE lagnname = ?", [validRegularUser.lagnname]);
            return res.status(200).send({
              success: true,
    ...
    if (adminUsers.length > 0) {
      ...
      if (isAdminPasswordValid) {
        ...
        // Create token payload for pure admin user (no activeusers record)
    ...
    // Check if the username belongs to an inactive account before returning "not found"
    try {
      const inactiveUsers = await query('SELECT lagnname FROM activeusers WHERE Active = "n"');
      const matchingInactive = inactiveUsers.filter(user => {
        const parts = user.lagnname.split(" ").filter(Boolean);
        const lastName = parts[0];
        const firstNameInitial = parts.length > 1 ? parts[1].charAt(0) : "";
        const constructedUsername = `${firstNameInitial}${lastName}`.toLowerCase();
    ...
    } catch (inactiveCheckErr) {
      // Non-fatal — fall through to generic "not found"
    }
...
router.post("/handleUserInfo", async (req, res) => {
  const { id, screenName, email, phone, esid, lagnname, decision, newPassword } = req.body;
  ...
  try {
    ...
    if (newPassword) {
      ...
      updateSql = `UPDATE activeusers 
       SET screen_name = ?, email = ?, phone = ?, lagnname = ?, redeemed = 1, Password = ?
       WHERE id = ?`;
      updateParams = [screenName, email, formattedPhone, lagnname, hashedPassword, id];
    } else {
      updateSql = `UPDATE activeusers 
       SET screen_name = ?, email = ?, phone = ?, lagnname = ?, redeemed = 1
       WHERE id = ?`;
      updateParams = [screenName, email, formattedPhone, lagnname, id];
    }
...
router.post("/searchByUserId", async (req, res) => {
  ...
  try {
    ...
    const placeholders = lagnnameList.map(() => "?").join(", ");
    const activeFilter = includeInactive ? "1=1" : "au.Active = 'y'";
    ...
    let queryText = `
        SELECT
            ...
            COALESCE(au.email, '') AS email,
            au.sa,
            COALESCE(sa_lookup.email, '') AS sa_email,
            au.ga,
            COALESCE(ga_lookup.email, '') AS ga_email,
            au.mga,
            COALESCE(mga_lookup.email, '') AS mga_email,
            au.rga,
            COALESCE(rga_lookup.email, '') AS rga_email
        FROM activeusers au
      LEFT JOIN activeusers sa_lookup ON au.sa = sa_lookup.lagnname AND sa_lookup.Active = 'y'
      LEFT JOIN activeusers ga_lookup ON au.ga = ga_lookup.lagnname AND ga_lookup.Active = 'y'
      LEFT JOIN activeusers mga_lookup ON au.mga = mga_lookup.lagnname AND mga_lookup.Active = 'y'
      LEFT JOIN activeusers rga_lookup ON au.rga = rga_lookup.lagnname AND rga_lookup.Active = 'y'
        WHERE ${activeFilter}
            AND (
                au.lagnname = ?
                OR (au.clname = 'RGA' AND au.lagnname = ?)
    ...
    if (userQuery.length === 0) {
      ...
      if (userinfoQuery.length === 0) {
        return res.status(404).json({ 
          success: false, 
          message: "We do not have this email in our system. Please check the spelling and try again, or reach out to your manager to have them reset your password." 
        });
      }
...
router.post("/verify-reset-code", async (req, res) => {
  ...
  try {
    ...
    const codeQuery = await query(
      'SELECT * FROM password_reset_tokens WHERE email = ? AND token = ? AND used = 0 AND expires_at > NOW() ORDER BY created_at DESC LIMIT 1',
      [email, code.trim().toUpperCase()]
    );
...
router.put("/update-password", async (req, res) => {
  const { userId, emailCode, newPassword, email } = req.body;

  if ((!userId && !email) || !emailCode || !newPassword) {
    return res.status(400).json({ 
      success: false, 
  ...
  try {
    ...
    const codeQuery = await query(
      'SELECT * FROM password_reset_tokens WHERE email = ? AND token = ? AND used = 0 AND expires_at > NOW() ORDER BY created_at DESC LIMIT 1',
      [email, emailCode.trim().toUpperCase()]
    );
    ...
    // Look up the user by email to get their id for password update
    let userQuery = await query(
      'SELECT id FROM activeusers WHERE email = ? AND Active = "y" LIMIT 1',
      [email]
    );
    if (userQuery.length === 0) {
      userQuery = await query(
        'SELECT au.id FROM usersinfo ui JOIN activeusers au ON ui.lagnname = au.lagnname AND ui.esid = au.esid WHERE ui.email = ? AND au.Active = "y" LIMIT 1',
        [email]
      );
    }
    if (userQuery.length === 0) {
    ...
    const resolvedUserId = userId || userQuery[0].id;
...
// ==========================================
// Token Refresh - Silent token renewal for PWA persistence
// ==========================================
router.post("/refresh", verifyToken, async (req, res) => {
  try {
    ...
    const users = await query("SELECT * FROM activeusers WHERE id = ?", [userId]);
    ...
    const admins = await query("SELECT * FROM admin_logins WHERE id = ?", [userId]);
    ...
    if (isAdmin) {
      ...
      tokenPayload = {
        ...
        headerPic: user.header_pic || null,
        Role: user.Role || null,
    ...
    await query("UPDATE user_tokens SET valid = 'n' WHERE userId = ? AND userToken = ?", [userId, req.headers.authorization?.replace('Bearer ', '')]);
    await query("INSERT INTO user_tokens (userId, userToken, valid) VALUES (?, ?, 'y')", [userId, newToken]);
