...
router.get('/', verifyToken, async (req, res) => {
  try {
    ...
    // 7. Process dynamic notifications using a SINGLE connection to avoid pool exhaustion
    ...
    if (dynamicNotifications.length > 0) {
      ...
      try {
        ...
        for (const notification of dynamicNotifications) {
          try {
            ...
            if (userCheckQuery.includes('WHERE')) {
              finalUserCheck = userCheckQuery.replace('WHERE', `WHERE a.id = ${parseInt(req.userId)} AND `);
            } else {
              finalUserCheck = userCheckQuery + ` WHERE a.id = ${parseInt(req.userId)}`;
            }

            const existsQuery = `SELECT EXISTS(${finalUserCheck}) as matches`;
            const userMatches = await db.queryOnConnection(conn, existsQuery, params);
        ...
        console.error('⚠️ [FETCH] Could not acquire connection for dynamic notifications:', connErr.message);
      } finally {
...
router.put('/read-all', verifyToken, async (req, res) => {
  try {
    ...
    // Check which dynamic notifications the user matches (single connection)
    ...
    if (dynamicNotifications.length > 0) {
      ...
      try {
        ...
        console.error('⚠️ [READ-ALL] Could not acquire connection for dynamic checks:', connErr.message);
      } finally {
        if (conn) conn.release();
      }
    }
    ...
    // Batch insert/update all read records in one query
    if (allNotificationIds.length > 0) {
      const values = allNotificationIds.map(nId => `(${parseInt(nId)}, ${parseInt(req.userId)}, true, NOW())`).join(',');
      await db.query(
        `INSERT INTO notification_reads (notification_id, user_id, is_read, created_at) VALUES ${values}
...
router.post('/subscribe', verifyToken, async (req, res) => {
  try {
    ...
    // Background cleanup: remove stale subscriptions for this user
    // This runs after saving the new one so it doesn't slow down the subscribe response
    setImmediate(async () => {
      try {
        const allUserSubs = await db.query(
        ...
        for (const sub of allUserSubs) {
          let subObj;
          try {
            subObj = typeof sub.subscription === 'string' ? JSON.parse(sub.subscription) : sub.subscription;
          ...
          // Skip the subscription we just saved/updated
          if (subObj.endpoint === subscriptionObj.endpoint) continue;
          
          // Test each other subscription with a silent push
          try {
            await webpush.sendNotification(subObj, JSON.stringify({
              title: '', body: '', silent: true,
              tag: 'subscription-check', data: { type: 'subscription_check' }
            }));
          } catch (error) {
        ...
        if (staleIds.length > 0) {
          ...
          console.log(`🧹 Cleaned ${staleIds.length} stale subscription(s) for user ${req.userId} on subscribe`);
        }
      } catch (cleanupError) {
        console.error('Background subscription cleanup error:', cleanupError);
      }
    });
...
async function sendPushNotification(userId, notification) {
  try {
    ...
    // Get user's push subscriptions (all devices) - include id for reliable cleanup
    ...
    console.log(`📤 Sending push to user ${userId}: ${results.length} subscription(s) found`);
    
    let successCount = 0;
    let failureCount = 0;
    const staleIds = []; // Track IDs of expired/gone subscriptions for batch cleanup
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      try {
        ...
        console.error(`Error parsing subscription for user ${userId}, record ${result.id}:`, parseError);
        failureCount++;
        staleIds.push(result.id);
        continue;
      }
      ...
      // Note: callers pass 'message' and 'link_url', so we map them to the expected fields
      const payload = JSON.stringify({
        ...
        link_url: notification.link_url || notification.url || '/',
        data: {
          notificationId: notification.id,
          url: notification.link_url || notification.url || '/',
          timestamp: Date.now(),
          ...notification.data
        },
      ...
      try {
        ...
        if (error.statusCode === 410 || error.statusCode === 404) {
          ...
          // Only log non-410 errors (410s are normal and expected for expired subs)
          console.error(`❌ Push error for user ${userId}, record ${result.id}: status ${error.statusCode}`);
        }
      }
    }
    
    // Batch cleanup all stale/expired subscriptions by ID (reliable, no JSON matching issues)
    if (staleIds.length > 0) {
      try {
        await db.query(
          `DELETE FROM push_subscriptions WHERE id IN (${staleIds.map(() => '?').join(',')})`,
        ...
        console.log(`🧹 Cleaned up ${staleIds.length} stale subscription(s) for user ${userId}`);
      } catch (deleteError) {
        console.error(`❌ Error batch-removing stale subscriptions:`, deleteError);
      }
    }
    
    console.log(`📊 Push results for user ${userId}: ${successCount} sent, ${failureCount} failed, ${staleIds.length} cleaned up`);
...
router.get('/debug-env', verifyToken, isAdmin, (req, res) => {
  res.json({
...
router.post('/send', verifyToken, isAdmin, async (req, res) => {
  try {
    ...
    // Note: notifyUser auto-wraps with type:'notification', so pass flat data
    if (global.notificationManager) {
      global.notificationManager.notifyUser(userId, {
        id: notificationId,
        title,
...
// Admin route to get push subscribers grouped by user (for send-to-user UI)
router.get('/admin/push-subscribers', verifyToken, isAdmin, async (req, res) => {
  try {
    ...
    let query = `
      SELECT 
        ...
        GROUP_CONCAT(
          CASE 
            WHEN JSON_EXTRACT(ps.subscription, '$.endpoint') LIKE '%web.push.apple.com%' THEN 'Safari'
            WHEN JSON_EXTRACT(ps.subscription, '$.endpoint') LIKE '%fcm.googleapis.com%' THEN 'Chrome/Edge'
            WHEN JSON_EXTRACT(ps.subscription, '$.endpoint') LIKE '%wns%' THEN 'Edge (Win)'
            WHEN JSON_EXTRACT(ps.subscription, '$.endpoint') LIKE '%mozilla.com%' THEN 'Firefox'
            ELSE 'Unknown'
          END
          SEPARATOR ', '
      ...
      FROM push_subscriptions ps
      INNER JOIN activeusers au ON ps.user_id = au.id
    `;

    const params = [];

    if (search) {
      query += ` WHERE (au.lagnname LIKE ? OR au.screen_name LIKE ? OR au.email LIKE ?)`;
      const searchTerm = `%${search}%`;
      params.push(searchTerm, searchTerm, searchTerm);
    }

    query += ` GROUP BY au.id, au.lagnname, au.clname, au.screen_name, au.profpic, au.email`;
    ...
    // Get total unique subscriber count
    let countQuery = `SELECT COUNT(DISTINCT ps.user_id) as total FROM push_subscriptions ps`;
    let countParams = [];

    if (search) {
      countQuery += ` INNER JOIN activeusers au ON ps.user_id = au.id WHERE (au.lagnname LIKE ? OR au.screen_name LIKE ? OR au.email LIKE ?)`;
      const searchTerm = `%${search}%`;
    ...
    res.status(500).json({ success: false, error: 'Failed to fetch push subscribers' });
  }
});

// Admin route to purge stale/expired push subscriptions
// Tests each subscription by sending a silent push - removes any that return 410/404
router.post('/admin/purge-stale-subscriptions', verifyToken, isAdmin, async (req, res) => {
  try {
    console.log('🧹 Admin purge stale subscriptions initiated');
    
    // Get all subscriptions
    const allSubs = await db.query('SELECT id, user_id, subscription, updated_at FROM push_subscriptions');
    
    if (!allSubs || allSubs.length === 0) {
      return res.json({ success: true, message: 'No subscriptions to check', purged: 0, total: 0 });
    }
    
    console.log(`🔍 Checking ${allSubs.length} subscriptions for staleness...`);
    
    const staleIds = [];
    const invalidIds = []; // Unparseable or missing endpoint
    
    for (const sub of allSubs) {
      // Check for invalid/unparseable subscriptions
      if (!sub.subscription) {
      ...
      // Try sending a minimal test payload to check if subscription is still valid
      try {
        // Send a tiny silent ping to verify the subscription endpoint
        await webpush.sendNotification(subscriptionObj, JSON.stringify({
          title: '',
          body: '',
          silent: true,
          tag: 'subscription-check',
          data: { type: 'subscription_check' }
        }));
        // If it succeeds, subscription is valid - skip
      } catch (error) {
        if (error.statusCode === 410 || error.statusCode === 404) {
          staleIds.push(sub.id);
        }
        // Other errors (e.g. 429 rate limit, 5xx) - don't remove, could be temporary
      }
    }
    
    // Batch delete all stale and invalid subscriptions
    const allBadIds = [...staleIds, ...invalidIds];
    let purgedCount = 0;
    
    if (allBadIds.length > 0) {
      const result = await db.query(
        `DELETE FROM push_subscriptions WHERE id IN (${allBadIds.map(() => '?').join(',')})`,
        allBadIds
      );
      purgedCount = result.affectedRows || allBadIds.length;
    }
    
    console.log(`🧹 Purge complete: ${purgedCount} removed (${staleIds.length} expired, ${invalidIds.length} invalid) out of ${allSubs.length} total`);
    
    res.json({
      success: true,
      message: `Purged ${purgedCount} stale subscriptions`,
      purged: purgedCount,
      expired: staleIds.length,
      invalid: invalidIds.length,
      total: allSubs.length,
      remaining: allSubs.length - purgedCount
    });
  } catch (error) {
    console.error('Error purging stale subscriptions:', error);
    res.status(500).json({ success: false, error: 'Failed to purge stale subscriptions' });
  }
});
...
router.get('/email-preferences', verifyToken, async (req, res) => {
  try {
    const rows = await db.query(
      'SELECT preference_type, enabled FROM email_preferences WHERE user_id = ?',
      [req.userId]
    );

    // Build preferences object with defaults (all enabled if no row exists)
    const prefs = {
      daily_reminder: true,
      weekly_report: true
    };
    ...
    for (const row of rows) {
      if (row.preference_type in prefs) {
        prefs[row.preference_type] = !!row.enabled;
      }
    }
...
router.put('/email-preferences', verifyToken, async (req, res) => {
  try {
    const { daily_reminder, weekly_report } = req.body;

    const updates = [];
    if (typeof daily_reminder === 'boolean') {
      updates.push({ type: 'daily_reminder', enabled: daily_reminder });
    }
    if (typeof weekly_report === 'boolean') {
      updates.push({ type: 'weekly_report', enabled: weekly_report });
    }
    ...
    for (const update of updates) {
      await db.query(
        `INSERT INTO email_preferences (user_id, preference_type, enabled)
         VALUES (?, ?, ?)
         ON DUPLICATE KEY UPDATE enabled = VALUES(enabled)`,
        [req.userId, update.type, update.enabled ? 1 : 0]
      );
    }
    ...
    const prefs = { daily_reminder: true, weekly_report: true };
    for (const row of rows) {
      if (row.preference_type in prefs) {
        prefs[row.preference_type] = !!row.enabled;
      }
