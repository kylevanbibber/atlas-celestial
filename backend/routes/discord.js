...
/**
 * @route GET /api/discord/sales/guilds
 * @desc Get available Discord guilds for the current user to post sales to
 * @access Private
 */
router.get('/sales/guilds', authMiddleware, async (req, res) => {
  try {
    const userId = req.user?.userId || req.userId;

    // Get user's discord_id
    const userRows = await db.query(
      'SELECT id, discord_id FROM activeusers WHERE id = ? LIMIT 1',
      [userId]
    );
    if (!userRows.length || !userRows[0].discord_id) {
    ...
    // Check every guild the bot is in for user membership (in parallel)
    const memberChecks = client.guilds.cache.map(async (guild) => {
      try {
        await guild.members.fetch(discordId);
        return guild; // user is a member
      } catch {
        return null; // user is not in this guild
      }
    });
    const results = await Promise.all(memberChecks);
    const sharedGuilds = results.filter(Boolean);

    if (!sharedGuilds.length) {
      return res.json({ success: true, data: [] });
    }

    // Look up configured default sales channel per guild from guild_configs
    const guildIds = sharedGuilds.map(g => g.id);
    const configuredChannels = await db.query(`
      SELECT guild_id, channel_id
      FROM guild_configs
      WHERE guild_id IN (?) AND channel_id IS NOT NULL
    `, [guildIds]);
    const defaultChannelMap = {};
    for (const row of configuredChannels) {
      if (!defaultChannelMap[row.guild_id]) {
        defaultChannelMap[row.guild_id] = row.channel_id;
      }
    }

    // Build response with all sendable text channels per guild
    const data = [];
    for (const guild of sharedGuilds) {
      const channels = guild.channels.cache
        .filter(ch => ch.isTextBased() && !ch.isVoiceBased() && ch.permissionsFor(guild.members.me)?.has('SendMessages'))
        .sort((a, b) => a.position - b.position)
      ...
      data.push({
        ...
        default_channel_id: defaultChannelMap[guild.id] || channels[0].id,
        channels,
      });
    }

    data.sort((a, b) => a.guild_name.localeCompare(b.guild_name));
...
/**
 * @route POST /api/discord/sales
 * @desc Manually add a sale (same as Discord bot /close)
 * @access Private
 */
const ALLOWED_LEAD_TYPES = ['union', 'credit_union', 'association', 'pos', 'ref', 'child_safe', 'free_will_kit', 'other'];

router.post('/sales', authMiddleware, async (req, res) => {
  try {
    const userId = req.user?.userId || req.userId;
    const { alp, refs = 0, lead_type, image_url = null, guild_id: reqGuildId = null, channel_id: reqChannelId = null } = req.body;

    // Validate
    if (!alp || parseFloat(alp) <= 0) {
      return res.status(400).json({ success: false, message: 'ALP must be greater than 0' });
    }
    ...
    // Get user details
    const userRows = await db.query(
      'SELECT id, lagnname, clname, profpic, discord_id FROM activeusers WHERE id = ? LIMIT 1',
      [userId]
    );
    if (!userRows.length) {
    ...
    // Eastern time timestamp (same format as Discord bot)
    const now = new Date();
    const easternTimestamp = now.toLocaleString('en-US', {
    ...
    const [datePart, timePart] = easternTimestamp.split(', ');
    const [month, day, year] = datePart.split('/');
    const formattedTs = `${year}-${month}-${day} ${timePart}`;
    const saleDate = `${year}-${month}-${day}`;

    // Resolve discord_user and guild_id for the insert
    const discordUser = user.discord_id || 'web';
    let guildId = reqGuildId || 'web';
    let channelId = reqChannelId || null;

    // If no guild was explicitly selected but user has discord_id, auto-resolve
    if (!reqGuildId && user.discord_id) {
      const prevSale = await db.query(
        `SELECT guild_id FROM discord_sales WHERE discord_user = ? AND guild_id IS NOT NULL AND guild_id != 'web' ORDER BY ts DESC LIMIT 1`,
        [user.discord_id]
      );
      if (prevSale.length && prevSale[0].guild_id) {
        guildId = prevSale[0].guild_id;
      } else {
        const guildConfig = await db.query(
          `SELECT gc.guild_id FROM guild_configs gc
           JOIN activeusers au ON au.id = gc.manager_id
           WHERE au.lagnname = (SELECT mga FROM activeusers WHERE id = ?)
           LIMIT 1`,
          [userId]
        );
        if (guildConfig.length && guildConfig[0].guild_id) {
          guildId = guildConfig[0].guild_id;
        }
      }
    }

    // If we have a guild but no channel, look up the channel from guild_configs
    if (guildId && guildId !== 'web' && !channelId) {
      const gcRows = await db.query(
        `SELECT channel_id FROM guild_configs WHERE guild_id = ? LIMIT 1`,
        [guildId]
      );
      if (gcRows.length && gcRows[0].channel_id) {
        channelId = gcRows[0].channel_id;
      }
    }
    ...
    const result = await db.query(
      `INSERT INTO discord_sales (discord_user, guild_id, alp, refs, lead_type, image_url, user_id, submission_id, ts)
       VALUES (?, ?, ?, ?, ?, ?, ?, NULL, ?)`,
      [discordUser, guildId, parseFloat(alp), parseInt(refs, 10), lead_type, image_url || null, userId, formattedTs]
    );

    // Update Daily_Activity
    await updateDailyActivityFromDiscordSales(userId, saleDate);

    // Get today's totals for daily sale context
    const totalResult = await db.query(
      `SELECT COALESCE(SUM(alp), 0) as total_alp, COUNT(*) as total_closes
       FROM discord_sales WHERE user_id = ? AND DATE(ts) = ?`,
      [userId, saleDate]
    );
    const totals = totalResult[0] || { total_alp: 0, total_closes: 1 };

    // Build the event object
    const event = {
      id: `close-${result.insertId}`,
      type: 'close',
      actor: {
        id: user.id,
        name: user.lagnname,
        profilePic: user.profpic || null,
        class: user.clname || null
      },
      data: {
        alp: parseFloat(alp),
        refs: parseInt(refs, 10),
        leadType: lead_type,
        imageUrl: image_url || null,
        dailySaleNumber: parseInt(totals.total_closes, 10) || 1,
        dailyAlpTotal: parseFloat(totals.total_alp) || 0
      },
    ...
    if (global.notificationManager) {
      global.notificationManager.broadcastAll({
        type: 'new_close',
        event
      });
    }

    // Post sale announcement to Discord channel if we have a valid guild/channel
    if (channelId && guildId !== 'web') {
      try {
        const client = require('../bot').getClient();
        if (client) {
          ...
          if (discordChannel && discordChannel.isTextBased()) {
            const leadTypeDisplay = lead_type.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
            ...
            const DAILY_MILESTONE_NAMES = {
              3: '🎩 Hat Trick', 4: '🍗 4 Piece Dinner', 5: '🖐️ Five Alive',
              6: '🥚 Half Dozen', 7: '☁️ Seventh Heaven', 8: '⛽ Octane',
              9: '🌤️ On Cloud 9', 10: '💎 Dime Piece'
            };
            const saleCount = parseInt(totals.total_closes, 10) || 1;
            const milestoneName = DAILY_MILESTONE_NAMES[saleCount] || (saleCount > 10 ? `🔥 ${saleCount} Sales Today` : null);

            const embed = new EmbedBuilder()
              .setTitle(milestoneName ? `${milestoneName}!` : '🎉 New Close Recorded!')
              .setColor(milestoneName ? '#f59e0b' : '#00ff00')
              .addFields(
                { name: '👤 Agent', value: user.lagnname, inline: true },
                { name: '💰 ALP', value: `$${parseFloat(alp).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, inline: true },
                { name: '🔗 Refs', value: String(parseInt(refs, 10)), inline: true },
                { name: '📋 Lead Type', value: leadTypeDisplay, inline: true },
                { name: '⏰ Time', value: displayTime, inline: true },
                { name: '📊 Today\'s Totals', value: `**ALP:** $${parseFloat(totals.total_alp).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}\n**Closes:** ${totals.total_closes}`, inline: false }
              )
              .setTimestamp()
              .setFooter({ text: 'Recorded via Atlas' });

            if (image_url) {
              embed.setImage(image_url);
            }

            const messageContent = user.discord_id
              ? (milestoneName ? `<@${user.discord_id}> just hit **${milestoneName}**! 🎉` : `<@${user.discord_id}> recorded a new close! 🎉`)
              : (milestoneName ? `**${user.lagnname}** just hit **${milestoneName}**! 🎉` : `**${user.lagnname}** recorded a new close! 🎉`);

            await discordChannel.send({ content: messageContent, embeds: [embed] });
            console.log(`[ManualSale] Posted sale embed to channel ${channelId}`);
          }
        }
      } catch (botErr) {
        // Don't fail the sale if bot posting fails
        console.error('[ManualSale] Failed to post to Discord channel:', botErr.message);
      }
    }
...
async function updateDailyActivityFromDiscordSales(userId, saleDate) {
  try {
    ...
    const existingRecord = await db.query(
      'SELECT * FROM Daily_Activity WHERE TRIM(agent) = TRIM(?) AND reportDate = ?',
    ...
    if (existingRecord && existingRecord.length > 0) {
      ...
      await db.query(`
        ...
        SET
          ...
          alp = ?,
          refs = ?
        WHERE TRIM(agent) = TRIM(?) AND reportDate = ?
      `, [
        newSales, 
        newAlp, 
        newRefs,
