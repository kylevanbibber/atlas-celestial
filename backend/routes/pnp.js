...
/**
 * GET /api/pnp/recap
 * Get PnP recap data by agent level (AGT, SA, GA, MGA)
 * Joins PnP rows to activeusers to determine clname, uses appropriate suffix/fields per level
 * Query params: date (required, mm/dd/yy format)
 */
router.get('/recap', verifyToken, async (req, res) => {
  try {
    ...
    // Find the previous year's December PnP date
    const selectedDate = new Date(
      (() => { const p = date.split('/'); const y = parseInt(p[2]); return `${2000 + (y < 50 ? y : y - 100)}-${p[0].padStart(2,'0')}-${p[1].padStart(2,'0')}`; })()
    );
    const prevYear = selectedDate.getFullYear() - 1;

    const decDatesRows = await db.query(`
      SELECT DISTINCT date FROM pnp
      WHERE MONTH(STR_TO_DATE(date, '%m/%d/%y')) = 12
        AND YEAR(STR_TO_DATE(date, '%m/%d/%y')) = ?
      ORDER BY STR_TO_DATE(date, '%m/%d/%y') DESC
      LIMIT 1
    `, [prevYear]);
    const prevDecDate = decDatesRows.length > 0 ? decDatesRows[0].date : null;

    // Helper: fetch PnP rows for a given date and suffix, joined to activeusers
    const fetchRecapRows = async (pnpDate, suffix) => {
      const rows = await db.query(`
        SELECT
          ...
          p.cur_ytd_life_net_submit,
          p.first_6_mo_net_submit,
          p.first_6_ytd_net,
          p.curr_mo_4mo_rate,
          au.clname,
          au.lagnname
        ...
        INNER JOIN activeusers au ON (
          (UPPER(TRIM(p.name_line)) = UPPER(TRIM(au.lagnname))
           OR UPPER(TRIM(au.lagnname)) LIKE CONCAT(UPPER(TRIM(p.name_line)), ' %'))
          AND ABS(DATEDIFF(STR_TO_DATE(p.esid, '%m/%d/%y'), STR_TO_DATE(au.esid, '%Y-%m-%d'))) <= 7
        )
        WHERE p.date = ?
          AND p.agent_num LIKE ?
          AND au.Active = 'y'
      `, [pnpDate, `%${suffix}`]);
      return rows;
    };

    // Fetch current date data for all suffixes
    const [rows1, rows2, rows3] = await Promise.all([
      fetchRecapRows(date, '-1'),
      fetchRecapRows(date, '-2'),
      fetchRecapRows(date, '-3')
    ]);

    // Fetch prev Dec data for YTD calculation
    let decRows1 = [], decRows2 = [], decRows3 = [];
    if (prevDecDate) {
      [decRows1, decRows2, decRows3] = await Promise.all([
        fetchRecapRows(prevDecDate, '-1'),
        fetchRecapRows(prevDecDate, '-2'),
        fetchRecapRows(prevDecDate, '-3')
      ]);
    }

    // Build lookup maps for prev Dec data by lagnname (lowercase)
    const buildDecLookup = (rows, field) => {
      const map = new Map();
      rows.forEach(r => {
        const key = String(r.lagnname || r.name_line).toLowerCase();
        const val = parseFloat(String(r[field] || '').replace(/[^0-9.-]/g, '')) || 0;
        map.set(key, val);
      });
      return map;
    };

    const decLookup1 = buildDecLookup(decRows1, 'curr_mo_net_submit');
    const decLookup2 = buildDecLookup(decRows2, 'first_6_mo_net_submit');
    const decLookup3Curr = buildDecLookup(decRows3, 'curr_mo_net_submit');
    const decLookup3First6 = buildDecLookup(decRows3, 'first_6_mo_net_submit');

    const parseNum = (v) => parseFloat(String(v || '').replace(/[^0-9.-]/g, '')) || 0;

    // Build AGT data from ALL -1 rows (all agents, not filtered by clname)
    const agt = rows1
      .map(r => {
        const key = String(r.lagnname).toLowerCase();
        const monthRecap = parseNum(r.curr_mo_net_submit);
        const ytdBase = parseNum(r.cur_ytd_life_net_submit);
        const prevDec = decLookup1.get(key) || 0;
        return { name_line: r.lagnname, esid: r.esid, curr_mo_4mo_rate: r.curr_mo_4mo_rate, month_recap: monthRecap, ytd_recap: ytdBase + prevDec };
      });

    // Build SA data from -2 rows where clname = 'SA'
    const sa = rows2
      .filter(r => String(r.clname).toUpperCase() === 'SA')
      .map(r => {
        const key = String(r.lagnname).toLowerCase();
        const monthRecap = parseNum(r.first_6_mo_net_submit);
        const ytdBase = parseNum(r.first_6_ytd_net);
        const prevDec = decLookup2.get(key) || 0;
        return { name_line: r.lagnname, esid: r.esid, curr_mo_4mo_rate: r.curr_mo_4mo_rate, month_recap: monthRecap, ytd_recap: ytdBase + prevDec };
      });

    // Build GA data from -3 rows where clname = 'GA'
    const ga = rows3
      .filter(r => String(r.clname).toUpperCase() === 'GA')
      .map(r => {
        const key = String(r.lagnname).toLowerCase();
        const monthRecap = parseNum(r.first_6_mo_net_submit);
        const ytdBase = parseNum(r.first_6_ytd_net);
        const prevDec = decLookup3First6.get(key) || 0;
        return { name_line: r.lagnname, esid: r.esid, curr_mo_4mo_rate: r.curr_mo_4mo_rate, month_recap: monthRecap, ytd_recap: ytdBase + prevDec };
      });

    // Build MGA data from -3 rows where clname IN ('MGA','RGA')
    const mga = rows3
      .filter(r => ['MGA','RGA'].includes(String(r.clname).toUpperCase()))
      .map(r => {
        const key = String(r.lagnname).toLowerCase();
        const monthRecap = parseNum(r.curr_mo_net_submit);
        const ytdBase = parseNum(r.cur_ytd_life_net_submit);
        const prevDec = decLookup3Curr.get(key) || 0;
        return { name_line: r.lagnname, esid: r.esid, curr_mo_4mo_rate: r.curr_mo_4mo_rate, month_recap: monthRecap, ytd_recap: ytdBase + prevDec };
      });

    // Sort all by name
    const sortByName = (a, b) => (a.name_line || '').localeCompare(b.name_line || '');
    agt.sort(sortByName);
    sa.sort(sortByName);
    ga.sort(sortByName);
    mga.sort(sortByName);
    ...
    return res.json({
      ...
      data: { agt, sa, ga, mga },
      date,
      prevDecDate: prevDecDate || null
    });
  } catch (err) {
    console.error('[pnp] /recap error:', err);
    return res.status(500).json({ success: false, message: 'Error fetching recap data' });
  }
});
...
router.get('/allotment', verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Fetch allotment issued data for this month
    let issuedData = [];
    try {
      issuedData = await db.query(
        `SELECT ai.*
         FROM allotment_issued ai
         INNER JOIN lead_drop_dates ldd ON ai.drop_date_id = ldd.id
         WHERE ldd.allotment_month = ? AND ldd.is_active = 1`,
        [targetMonthFormatted]
      );
    } catch (issuedError) {
      console.error('Error fetching allotment issued data:', issuedError);
    }
    ...
    const responseData = {
      ...
      issuedData: issuedData, // Lead issuance records for F90 tab
...
/**
 * GET /api/pnp/allotment-issued?month=YYYY-MM
 * Get all lead issuance records for a given allotment month
 */
router.get('/allotment-issued', verifyToken, async (req, res) => {
  try {
    ...
    const records = await db.query(
      `SELECT ai.*
       ...
       INNER JOIN lead_drop_dates ldd ON ai.drop_date_id = ldd.id
       WHERE ldd.allotment_month = ? AND ldd.is_active = 1
       ORDER BY ai.agent_id, ai.drop_date_id, ai.lead_type`,
      [month]
    );

    res.json({ success: true, data: records });
  } catch (error) {
    console.error('Error fetching allotment issued data:', error);
    res.status(500).json({ success: false, message: 'Error fetching issuance data', error: error.message });
  }
});

/**
 * PUT /api/pnp/allotment-issued
 * Upsert a lead issuance record for an agent/drop/lead type
 */
router.put('/allotment-issued', verifyToken, async (req, res) => {
  try {
    const { user } = req;
    ...
    const { agentId, dropDateId, leadType, quantity } = req.body;

    if (!agentId || !dropDateId || !leadType) {
      return res.status(400).json({ success: false, message: 'agentId, dropDateId, and leadType are required' });
    }

    const validTypes = ['POS', 'HC', 'VN', 'Dcard'];
    if (!validTypes.includes(leadType)) {
      return res.status(400).json({ success: false, message: `leadType must be one of: ${validTypes.join(', ')}` });
    }

    const qty = parseInt(quantity) || 0;

    await db.query(
      `INSERT INTO allotment_issued (agent_id, drop_date_id, lead_type, quantity, created_by)
       VALUES (?, ?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE
         quantity = VALUES(quantity),
         updated_at = CURRENT_TIMESTAMP`,
      [agentId, dropDateId, leadType, qty, user.userId]
    );

    res.json({ success: true, message: 'Issuance record saved' });
  } catch (error) {
    console.error('Error saving allotment issued data:', error);
    res.status(500).json({ success: false, message: 'Error saving issuance data', error: error.message });
  }
});
