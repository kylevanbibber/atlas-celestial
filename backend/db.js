...
pool.on("error", (err) => {
  ...
  if (err.code === "PROTOCOL_CONNECTION_LOST" || err.code === "ECONNRESET") {
    ...
    console.error("Unhandled database pool error:", err.code || err.message);
  }
});
  ...
  return new Promise((resolve, reject) => {
    ...
    pool.getConnection((connErr, connection) => {
      ...
      connection.query(sql, params, (err, results) => {
        ...
        if (err) {
          const sqlText = typeof sql === 'string' ? sql.trim().substring(0, 200) : (sql.sql || '').trim().substring(0, 200);
          console.error('[DB] ❌ Query error after', totalMs, 'ms:', err.code || err.message, '| SQL:', sqlText);
          return reject(err);
        }
        resolve(results);
      });
...
// Run a query on an existing connection (no pool acquire/release)
const queryOnConnection = (connection, sql, params) => {
  return new Promise((resolve, reject) => {
    connection.query(sql, params, (err, results) => {
...
module.exports = { pool, getConnection, query, queryOnConnection };
