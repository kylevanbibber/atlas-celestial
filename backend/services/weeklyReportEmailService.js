  ...
  try {
    ...
    const fetchForSuffix = async (suffix) => {
      const q = `
        ...
        FROM pnp
        WHERE (TRIM(name_line) = TRIM(?) OR TRIM(?) LIKE CONCAT(TRIM(name_line), ' %'))
          AND agent_num LIKE ?
...
async function generateWeeklyReport(user) {
  ...
  try {
    ...
    `, [lagnname, monthStart, monthEnd]);
      ...
      // For SA, check SA column (their name appears as SA on associates they manage)
      const codesThisMonth = await db.query(`
        SELECT COUNT(*) as count
        FROM associates
        WHERE (SA = ? OR GA = ? OR MGA = ?)
        AND PRODDATE BETWEEN ? AND ?
      `, [lagnname, lagnname, lagnname, monthStart, monthEnd]);
      codesMTD = codesThisMonth[0]?.count || 0;
    } else if (isGA) {
      // For GA, check GA column
      const codesThisMonth = await db.query(`
        SELECT COUNT(*) as count
        FROM associates
        WHERE (GA = ? OR MGA = ?)
        AND PRODDATE BETWEEN ? AND ?
      `, [lagnname, lagnname, monthStart, monthEnd]);
      codesMTD = codesThisMonth[0]?.count || 0;
    }

    // 2. Count of pending (not yet coded) from last 45 days - use appropriate column based on role
    let pendingQuery;
    let pendingParams;
    if (isSA) {
      pendingQuery = `SELECT COUNT(*) as count FROM pending WHERE (SA = ? OR GA = ? OR MGA = ?) AND PendingDate >= ? AND LagnName NOT IN (SELECT LagnName FROM associates WHERE LagnName IS NOT NULL)`;
      pendingParams = [lagnname, lagnname, lagnname, fortyFiveDaysAgo];
    } else if (isGA) {
      pendingQuery = `SELECT COUNT(*) as count FROM pending WHERE (GA = ? OR MGA = ?) AND PendingDate >= ? AND LagnName NOT IN (SELECT LagnName FROM associates WHERE LagnName IS NOT NULL)`;
      pendingParams = [lagnname, lagnname, fortyFiveDaysAgo];
    } else {
      pendingQuery = `SELECT COUNT(*) as count FROM pending WHERE MGA = ? AND PendingDate >= ? AND LagnName NOT IN (SELECT LagnName FROM associates WHERE LagnName IS NOT NULL)`;
      pendingParams = [lagnname, fortyFiveDaysAgo];
    }
    const pendingCount = await db.query(pendingQuery, pendingParams);
    ...
    // 6. Associates from last month - use appropriate column based on role
    let lastMonthQuery;
    let lastMonthParams;
    if (isSA) {
      lastMonthQuery = `SELECT COUNT(*) as count FROM associates WHERE (SA = ? OR GA = ? OR MGA = ?) AND PRODDATE BETWEEN ? AND ?`;
      lastMonthParams = [lagnname, lagnname, lagnname, lastMonthStart, lastMonthEnd];
    } else if (isGA) {
      lastMonthQuery = `SELECT COUNT(*) as count FROM associates WHERE (GA = ? OR MGA = ?) AND PRODDATE BETWEEN ? AND ?`;
      lastMonthParams = [lagnname, lagnname, lastMonthStart, lastMonthEnd];
    } else {
      lastMonthQuery = `SELECT COUNT(*) as count FROM associates WHERE MGA = ? AND PRODDATE BETWEEN ? AND ?`;
      lastMonthParams = [lagnname, lastMonthStart, lastMonthEnd];
    }
    const associatesLastMonth = await db.query(lastMonthQuery, lastMonthParams);

    const codesLastMonth = associatesLastMonth[0]?.count || 0;

    // 7. VIPs from last month - use appropriate column based on role
    let vipsQuery;
    let vipsParams;
    if (isSA) {
      vipsQuery = `SELECT COUNT(*) as count FROM VIPs WHERE (TRIM(sa) = TRIM(?) OR TRIM(ga) = TRIM(?) OR TRIM(mga) = TRIM(?)) AND YEAR(vip_month) = ? AND MONTH(vip_month) = ?`;
      vipsParams = [lagnname, lagnname, lagnname, lastMonthStart.getFullYear(), lastMonthStart.getMonth() + 1];
    } else if (isGA) {
      vipsQuery = `SELECT COUNT(*) as count FROM VIPs WHERE (TRIM(ga) = TRIM(?) OR TRIM(mga) = TRIM(?)) AND YEAR(vip_month) = ? AND MONTH(vip_month) = ?`;
      vipsParams = [lagnname, lagnname, lastMonthStart.getFullYear(), lastMonthStart.getMonth() + 1];
    } else {
      vipsQuery = `SELECT COUNT(*) as count FROM VIPs WHERE TRIM(mga) = TRIM(?) AND YEAR(vip_month) = ? AND MONTH(vip_month) = ?`;
      vipsParams = [lagnname, lastMonthStart.getFullYear(), lastMonthStart.getMonth() + 1];
    }
    const vipsLastMonth = await db.query(vipsQuery, vipsParams);
...
function getFirstName(user) {
  ...
  // Try screen_name first (format: "First Last")
  if (user.screen_name && user.screen_name.trim()) {
    const parts = user.screen_name.trim().split(/\s+/);
 ...
 */
function formatSummaryEmail(countsByClname, successCount, failCount, totalRecipients, optedOut = 0) {
  const now = new Date();
...
<body>
  <div class="container">
    ...
    ${optedOut > 0 ? `
    <div class="summary-box" style="border-left-color: #95a5a6;">
      <div class="summary-label">Opted Out</div>
      <div class="summary-value" style="color: #95a5a6;">${optedOut}</div>
    </div>
    ` : ''}
 ...
 * @param {string} triggeredBy - 'cron', 'manual', or 'test'
 * @param {number|null} triggeredByUserId - ID of the user who triggered (for manual/test)
 */
async function sendWeeklyReports(triggeredBy = 'cron', triggeredByUserId = null) {
  let runId = null;
  ...
  try {
    ...
    const runResult = await db.query(
      `INSERT INTO weekly_report_runs (triggered_by, triggered_by_user_id, started_at, status)
       VALUES (?, ?, NOW(), 'running')`,
    ...
    const recipients = await db.query(`
      SELECT id, lagnname, email, clname
      FROM activeusers
      WHERE Active = 'y'
    ...
    // Filter out users who opted out of weekly report emails
    const optedOutRows = await db.query(`
      SELECT user_id FROM email_preferences
      WHERE preference_type = 'weekly_report' AND enabled = 0
    `);
    const optedOutIds = new Set(optedOutRows.map(r => r.user_id));
    const filteredRecipients = recipients.filter(u => !optedOutIds.has(u.id));
    const skippedOptOut = recipients.length - filteredRecipients.length;

    logger.info(`Found ${recipients.length} eligible recipients, ${skippedOptOut} opted out, sending to ${filteredRecipients.length}`);

    // Update run with total count (after opt-out filtering)
    await db.query(
      'UPDATE weekly_report_runs SET total_recipients = ? WHERE id = ?',
      [filteredRecipients.length, runId]
    );
    ...
    // Create email batch for unified log
    const weeklySubject = `Weekly Production Report - ${new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}`;
    const batchId = await emailService.createBatch('weekly_report', weeklySubject, { runId, triggeredBy });

    // Process recipients in concurrent batches for speed
    const BATCH_SIZE = 5; // Generate + send 5 reports concurrently (each report does multiple DB/API calls)
    ...
    for (let i = 0; i < filteredRecipients.length; i += BATCH_SIZE) {
      ...
      const results = await Promise.allSettled(
        batch.map(async (user) => {
          ...
          await emailService.sendEmail(user.email, weeklySubject, emailHtml, {
            batchId, source: 'weekly_report', recipientName: user.lagnname
          });

          // Log to DB
          try {
            const reportSummary = {
              codesMTD: reportData.codesMTD,
              hiresMTD: reportData.hiresMTD,
              pendingNotCoded: reportData.pendingNotCoded,
              hireToCodeRatio: reportData.hireToCodeRatio,
              mgaTeamALP: reportData.mgaTeamALP,
              rgaTeamALP: reportData.rgaTeamALP,
              personalProduction: reportData.personalProduction,
              codesLastMonth: reportData.codesLastMonth,
              vipsLastMonth: reportData.vipsLastMonth
            };
            await db.query(
              `INSERT INTO weekly_report_recipients (run_id, user_id, lagnname, clname, email, status, sent_at, report_data)
               VALUES (?, ?, ?, ?, ?, 'sent', NOW(), ?)`,
              [runId, user.id, user.lagnname, user.clname, user.email, JSON.stringify(reportSummary)]
            );
          } catch (logErr) {
            logger.warn(`Failed to log recipient send for ${user.lagnname}:`, logErr);
          }
      ...
      results.forEach((result, idx) => {
        ...
        if (result.status === 'fulfilled') {
          ...
          logger.error(`Failed to send report to ${user.lagnname}:`, result.reason);

          // Log failure to DB
          db.query(
            `INSERT INTO weekly_report_recipients (run_id, user_id, lagnname, clname, email, status, error_message)
             VALUES (?, ?, ?, ?, ?, 'failed', ?)`,
            [runId, user.id, user.lagnname, user.clname, user.email, result.reason?.message || 'Unknown error']
          ).catch(logErr => {
            logger.warn(`Failed to log recipient failure for ${user.lagnname}:`, logErr);
          });
        }
      });

      // Log progress
      logger.info(`[WeeklyReport] Progress: ${Math.min(i + BATCH_SIZE, filteredRecipients.length)}/${filteredRecipients.length} processed`);
    ...
    try {
      await db.query(
        `UPDATE weekly_report_runs 
         SET completed_at = NOW(), success_count = ?, fail_count = ?, status = 'completed', 
             summary = ?
         WHERE id = ?`,
        [successCount, failCount, JSON.stringify(countsByClname), runId]
      );
    } catch (logErr) {
      logger.warn('Failed to update run status:', logErr);
    }
    
    // Send summary email to kvanbibber@ariasagencies.com
    try {
      const summaryHtml = formatSummaryEmail(countsByClname, successCount, failCount, filteredRecipients.length, skippedOptOut);
      const summarySubject = `Weekly Report Summary - ${new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}`;
      await emailService.sendEmail('kvanbibber@ariasagencies.com', summarySubject, summaryHtml, { skipLog: true });
      logger.info('✅ Summary email sent to kvanbibber@ariasagencies.com');
    } catch (error) {
      logger.error('❌ Failed to send summary email:', error);
    }
    
    return { successCount, failCount, total: filteredRecipients.length, optedOut: skippedOptOut, countsByClname, runId };
  } catch (error) {
    ...
    if (runId) {
      try {
        await db.query(
          `UPDATE weekly_report_runs SET completed_at = NOW(), status = 'failed' WHERE id = ?`,
          [runId]
        );
      } catch (logErr) {
        logger.warn('Failed to mark run as failed:', logErr);
      }
    }
...
/**
 * Send a single test weekly report and log it
 * @param {object} user - The user object
 * @param {number|null} triggeredByUserId - Admin user who triggered the test
 * @returns {object} Report data and run info
 */
async function sendTestWeeklyReport(user, triggeredByUserId = null) {
  let runId = null;

  try {
    // Create a test run entry
    const runResult = await db.query(
      `INSERT INTO weekly_report_runs (triggered_by, triggered_by_user_id, started_at, total_recipients, status)
       VALUES ('test', ?, NOW(), 1, 'running')`,
      [triggeredByUserId]
    );
    runId = runResult.insertId;
    ...
    const testBatchId = await emailService.createBatch('weekly_report', subject, { runId, triggeredBy: 'test' });
    await emailService.sendEmail(user.email, subject, emailHtml, {
      batchId: testBatchId, source: 'weekly_report', recipientName: user.lagnname
    });
    await emailService.completeBatch(testBatchId, 1, 0);
    ...
    await db.query(
      `UPDATE weekly_report_runs 
       SET completed_at = NOW(), success_count = 1, fail_count = 0, status = 'completed'
       WHERE id = ?`,
      [runId]
    );

    return { reportData, runId };
  } catch (error) {
    if (runId) {
      try {
        await db.query(
          `INSERT INTO weekly_report_recipients (run_id, user_id, lagnname, clname, email, status, error_message)
           VALUES (?, ?, ?, ?, ?, 'failed', ?)`,
          [runId, user.id, user.lagnname, user.clname, user.email, error.message]
        );
        await db.query(
          `UPDATE weekly_report_runs SET completed_at = NOW(), fail_count = 1, status = 'failed' WHERE id = ?`,
          [runId]
        );
      } catch (logErr) {
        logger.warn('Failed to log test failure:', logErr);
      }
    }
    throw error;
  }
