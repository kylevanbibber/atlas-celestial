  ...
  for (const u of users) {
    try {
      ...
      const rows = await db.query(
        `SELECT 1 FROM amore_data WHERE TRIM(MGA) = TRIM(?) AND MORE_Date = ? LIMIT 1`,
...
async function updateDailyActivityFromDiscordSale(userId, saleDate, alp, refs) {
  try {
    ...
    const existingRecordQuery = 'SELECT * FROM Daily_Activity WHERE TRIM(agent) = TRIM(?) AND reportDate = ?';
...
async function handleCloseCommand(interaction) {
  ...
  try {
    ...
    const userQuery = `
      SELECT id, lagnname, clname, profpic
      FROM activeusers
    ...
    // Calculate today's totals BEFORE broadcast so we can include daily context
    ...
    // Broadcast new close event to all connected WebSocket users for real-time activity feed
    if (global.notificationManager && insertResult.insertId) {
      try {
        global.notificationManager.broadcastAll({
          type: 'new_close',
          event: {
            id: `close-${insertResult.insertId}`,
            type: 'close',
            actor: {
              id: user.id,
              name: user.lagnname || 'Unknown',
              profilePic: user.profpic || null,
              class: user.clname || null
            },
            data: {
              alp: parseFloat(alp) || 0,
        ...
        console.log(`[BOT] 🎯 ✅ Broadcasted new_close to WebSocket clients`);
      } catch (broadcastErr) {
        console.error(`[BOT] 🎯 ⚠️ Failed to broadcast new_close:`, broadcastErr.message);
      }
    }
    ...
    try {
      ...
      // Milestone names for daily sale streaks
      ...
      if (channel && channel.isTextBased()) {
        ...
        const messageContent = milestoneName
          ? `<@${discordUserId}> just hit **${milestoneName}**! 🎉`
          : `<@${discordUserId}> recorded a new close! 🎉`;
        await channel.send({ 
  ...
  try {
    ...
    if (!existsRows || existsRows.length === 0) {
      ...
      const updateSql = `
        ...
        WHERE TRIM(MGA) = TRIM(?) AND MORE_Date = ?
      `;
